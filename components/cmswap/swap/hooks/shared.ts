...
export function normalizeTokenPair<T extends SwapTokenBase>(
    ...
    if (!tokens.length) throw new Error('normalizeTokenPair requires at least one token in the list.');
    ...
    const isNativeWrappedPair = (isTokenANative && tokenBUpper === wrappedUpper) || (isTokenBNative && tokenAUpper === wrappedUpper);
    return {tokenAValue, tokenBValue, isSameToken, isNativeWrappedPair, isTokenANative, isTokenBNative}
}
