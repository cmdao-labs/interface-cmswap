...
export default function PositionsUnified({ setIsLoading, setErrMsg, }: {
    setIsLoading: React.Dispatch<React.SetStateAction<boolean>>,
    setErrMsg: React.Dispatch<React.SetStateAction<WriteContractErrorType | null>>,
}) {
    const { address } = useAccount()
    const { chainId, tokens, POSITION_MANAGER, v3FactoryContract, positionManagerContract, erc20ABI, kap20ABI, v3PoolABI, chainCfg, toWrapped, isNative, isKap20Token, decimalsOf } = useLiquidityChain()
    const [txupdate, setTxupdate] = React.useState("")
    const [tokenA, setTokenA] = React.useState<{ name: string, value: '0xstring', logo: string }>(tokens[0] as any)
    ...
    const setAlignedAmountB = useDebouncedCallback(async (_amountA: string) => {
        ...
        const token0 = poolState[0].result !== undefined ? (poolState[0].result as '0xstring') : ('' as '0xstring')
        const sqrtPriceX96 = poolState[1].result !== undefined ? (poolState[1].result[0] as bigint) : BigInt(0)
        const tick = poolState[1].result !== undefined ? (poolState[1].result[1] as number) : 0
        const liquidity = poolState[2].result !== undefined ? (poolState[2].result as bigint) : BigInt(0)
        const dec0 = decimalsOf(token0)
        const dec1 = String(token0).toUpperCase() === tokenAvalue.toUpperCase() ? decimalsOf(tokenBvalue) : decimalsOf(tokenAvalue)
        const Token0 = new UniToken(chainId, token0, dec0)
        const Token1 = String(token0).toUpperCase() === tokenAvalue.toUpperCase() ? new UniToken(chainId, tokenBvalue, dec1) : new UniToken(chainId, tokenAvalue, dec1)
        ...
        if (String(token0).toUpperCase() === tokenAvalue.toUpperCase()) {
            const singleSidePositionToken0 = Position.fromAmount0({pool, tickLower: Number(lowerTick), tickUpper: Number(upperTick), amount0: String(parseUnits(_amountA || '0', decimalsOf(tokenA.value))) as BigintIsh, useFullPrecision: true})
            setAmountB(formatUnits(singleSidePositionToken0.mintAmounts.amount1 as unknown as bigint, decimalsOf(tokenB.value)))
        } else {
            const singleSidePositionToken1 = Position.fromAmount1({pool, tickLower: Number(lowerTick), tickUpper: Number(upperTick), amount1: String(parseUnits(_amountA || '0', decimalsOf(tokenA.value))) as BigintIsh})
            setAmountB(formatUnits(singleSidePositionToken1.mintAmounts.amount0 as unknown as bigint, decimalsOf(tokenB.value)))
        }
    }, 700)
    const setAlignedAmountA = useDebouncedCallback(async (_amountB: string) => {
        ...
        const pool = new Pool(Token0, Token1, Number(feeSelect), sqrtPriceX96.toString(), liquidity.toString(), tick)
        if (String(token0).toUpperCase() === tokenAvalue.toUpperCase()) {
            const singleSidePositionToken0 = Position.fromAmount0({pool, tickLower: Number(lowerTick), tickUpper: Number(upperTick), amount0: String(parseUnits(_amountB || '0', decimalsOf(tokenB.value))) as BigintIsh, useFullPrecision: true})
            setAmountA(formatUnits(singleSidePositionToken0.mintAmounts.amount1 as unknown as bigint, decimalsOf(tokenA.value)))
        } else {
            const singleSidePositionToken1 = Position.fromAmount1({pool, tickLower: Number(lowerTick), tickUpper: Number(upperTick), amount1: String(parseUnits(_amountB || '0', decimalsOf(tokenB.value))) as BigintIsh})
            setAmountA(formatUnits(singleSidePositionToken1.mintAmounts.amount0 as unknown as bigint, decimalsOf(tokenA.value)))
        }
    }, 700)
    const getBalanceOfAB = async (_tokenAvalue: '0xstring', _tokenBvalue: '0xstring') => {
        ...
        if (isNative(_tokenAvalue)) {
            nativeBal && setTokenABalance(formatEther(nativeBal.value))
        } else {
            bal[0].result !== undefined && setTokenABalance(formatUnits(bal[0].result as bigint, decimalsOf(_tokenAvalue)))
        }
        if (isNative(_tokenBvalue)) {
            nativeBal && setTokenBBalance(formatEther(nativeBal.value))
        } else {
            bal[1].result !== undefined && setTokenBBalance(formatUnits(bal[1].result as bigint, decimalsOf(_tokenBvalue)))
        }
    }
    const increaseLiquidity = async (_tokenId: bigint) => {
        ...
        try {
            ...
            } else if (isNative(tokenB.value) && amountB) {
                if (chainCfg.wrappedNative?.address) {
                    const h = await sendTransaction(config, { to: chainCfg.wrappedNative.address, value: parseUnits(amountB, decB) })
                    await waitForTransactionReceipt(config, { hash: h })
                }
            }
            ...
            if (allowanceA < parseUnits(amountA || '0', decA)) {
                const { request } = await simulateContract(config, { ...erc20ABI, address: tokenA.value, functionName: 'approve', args: [POSITION_MANAGER, parseUnits(amountA || '0', decA)] })
            ...
            if (allowanceB < parseUnits(amountB || '0', decB)) {
                const { request } = await simulateContract(config, { ...erc20ABI, address: tokenB.value, functionName: 'approve', args: [POSITION_MANAGER, parseUnits(amountB || '0', decB)] })
                const h = await writeContract(config, request)
                await waitForTransactionReceipt(config, { hash: h })
            }
            const { request } = await simulateContract(config, {
                ...
                args: [{
                    tokenId: _tokenId,
                    amount0Desired: parseUnits(amountA || '0', decA),
                    amount1Desired: parseUnits(amountB || '0', decB),
                    amount0Min: BigInt(0),
                    amount1Min: BigInt(0),
        ...
        try {
            ...
            if (wrappedAddr) {
                if (tokenA.value.toUpperCase() === wrappedAddr.toUpperCase()) {
                    const { request } = await simulateContract(config, { ...(chainCfg as any).wrappedNative, functionName: 'withdraw', args: [result[1] as bigint] })
                    const hh = await writeContract(config, request)
                    await waitForTransactionReceipt(config, { hash: hh })
                } else if (tokenB.value.toUpperCase() === wrappedAddr.toUpperCase()) {
                    const { request } = await simulateContract(config, { ...(chainCfg as any).wrappedNative, functionName: 'withdraw', args: [result[0] as bigint] })
                    const hh = await writeContract(config, request)
                    await waitForTransactionReceipt(config, { hash: hh })
                }
            }
    ...
    React.useEffect(() => {
        const fetchPositions = async () => {
            ...
            const init: any = { contracts: [] as any[] }
            ...
            const myPosition: MyPosition[] = (await Promise.all(tokenIdMyPosition.map(async (obj, index) => {
                ...
                const slot0 = await readContract(config, { ...v3PoolABI, address: pairAddr as '0xstring', functionName: 'slot0' })
                ...
                const _currPrice = (Number((slot0 as any)[0]) / (2 ** 96)) ** 2
                const lowerTickNum = Number(pos[5])
                const upperTickNum = Number(pos[6])
                const _lowerPrice = Math.pow(1.0001, lowerTickNum)
                const _upperPrice = Math.pow(1.0001, upperTickNum)
                ...
                const _fee0 = (qouteFee as any).result[0]
                const _fee1 = (qouteFee as any).result[1]
                ...
                const lowerPriceNum = _lowerPrice
                const upperPriceNum = _upperPrice
                const currPriceNum = _currPrice
                const fee0 = _fee1
                const fee1 = _fee0
                return {Id: Number(obj.result), Name: String(metadata.name), Image: String(metadata.image), FeeTier: Number(pos[4]), Pair: pairAddr as string, Token0Addr: token0addr as string, Token1Addr: token1addr as string, Token0: token0name, Token1: token1name, Amount0: amount0, Amount1: amount1, MinPrice: lowerPriceNum, MaxPrice: upperPriceNum, CurrPrice: currPriceNum, LowerTick: lowerTickNum, UpperTick: upperTickNum, Liquidity: liquidity, Fee0: Number(fee0) / 1e18, Fee1: Number(fee1) / 1e18}
            }))).filter((obj) => { return Number(obj.Liquidity) !== 0 }).reverse()
            setPosition(myPosition)
        }
        setAmountA("")
        setAmountB("")
        address !== undefined && fetchPositions()
    }, [address, tokenA, tokenB, feeSelect, txupdate, positionManagerContract, v3FactoryContract, v3PoolABI, erc20ABI])
    const clearState = () => {
        setTokenA(tokens[0] as any)
                        ...
                        {position.map(obj =>
                            <div key={Number(obj.Id)} className="mb-4 w-full bg-[#0a0b1e]/80 border border-[#00ff9d]/10 rounded-xl gap-2 flex flex-col items-start">
                                ...
                                <div className="w-full mb-4 py-1 px-6 gap-2 flex flex-row items-start justify-start flex-wrap">
                                    ...
                                    {(Number(obj.Fee0) > 0 || Number(obj.Fee1) > 0) && <Button variant="outline" className="bg-[#00ff9d]/10 hover:bg-[#00ff9d]/20 text-[#00ff9d] border border-[#00ff9d]/30 rounded-md cursor-pointer" onClick={() => collectFee(BigInt(obj.Id))}>Collect fee</Button>}
                                </div>
                            </div>
                        )}
                    </>
