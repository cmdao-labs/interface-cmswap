...
export default function PositionsUnified({ setIsLoading, setErrMsg, }: {
  ...
  const calcAmount0 = (
    ...
    const math = mathCurrentPrice <= mathPriceLower
      ? liquidity * ((mathPriceUpper - mathPriceLower) / (mathPriceLower * mathPriceUpper))
      : liquidity * ((mathPriceUpper - mathCurrentPrice) / (mathCurrentPrice * mathPriceUpper))
  ...
  const calcAmount1 = (
    ...
    const mathPriceUpper = Math.sqrt(priceUpper / decimalAdjustment)
    const mathPriceLower = Math.sqrt(priceLower / decimalAdjustment)
    const math = mathCurrentPrice >= mathPriceUpper
      ? liquidity * (mathPriceUpper - mathPriceLower)
      : liquidity * (mathCurrentPrice - mathPriceLower)
    const adjustedMath = math > 0 ? math : 0
    return adjustedMath
  }
  ...
  const setAlignedAmountB = useDebouncedCallback(async (_amountA: string) => {
    ...
    const Token1 = String(token0).toUpperCase() === tokenAvalue.toUpperCase()
      ? new UniToken(chainId, tokenBvalue, dec1)
      : new UniToken(chainId, tokenAvalue, dec1)
    ...
    if (String(token0).toUpperCase() === tokenAvalue.toUpperCase()) {
      const singleSidePositionToken0 = Position.fromAmount0({
        ...
        amount0: String(parseUnits(_amountA || '0', decimalsOf(tokenA.value))) as BigintIsh,
      ...
      const singleSidePositionToken1 = Position.fromAmount1({
        ...
        tickLower: Number(lowerTick),
        tickUpper: Number(upperTick),
        amount1: String(parseUnits(_amountA || '0', decimalsOf(tokenA.value))) as BigintIsh,
      })
  ...
  const setAlignedAmountA = useDebouncedCallback(async (_amountB: string) => {
    ...
    if (String(token0).toUpperCase() === tokenAvalue.toUpperCase()) {
      const singleSidePositionToken0 = Position.fromAmount0({
        ...
        tickLower: Number(lowerTick),
        tickUpper: Number(upperTick),
        amount0: String(parseUnits(_amountB || '0', decimalsOf(tokenB.value))) as BigintIsh,
        useFullPrecision: true,
      })
      ...
      const singleSidePositionToken1 = Position.fromAmount1({
        ...
        tickLower: Number(lowerTick),
        tickUpper: Number(upperTick),
        amount1: String(parseUnits(_amountB || '0', decimalsOf(tokenB.value))) as BigintIsh,
      })
    ...
    const fetchPositions = async () => {
      ...
      const myPosition: MyPosition[] = (await Promise.all(tokenIdMyPosition.map(async (obj, index) => {
        ...
        return {
          ...
          Amount0: amount0,
          Amount1: amount1,
          MinPrice: lowerPriceNum,
          MaxPrice: upperPriceNum,
          CurrPrice: currPriceNum,
          LowerTick: lowerTickNum,
          UpperTick: upperTickNum,
          Liquidity: liquidity,
