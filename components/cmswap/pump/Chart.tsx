...
type RawPoint = { time: number; /*milliseconds (epoch)*/ price: number; volume: number; };
type Candle = { time: number; /*seconds (epoch)*/ open: number; high: number; low: number; close: number; volume: number; lastTs?: number; };
const TIMEFRAMES = [
...
function aggregateCandlesWithFill(points: RawPoint[], intervalMs: number): Candle[] {
    ...
    for (let t = start; t <= end; t += intervalMs) {
        ...
        if (bucket.length === 0) {
            candles.push({time: Math.floor(t / 1000), open: previousClose, high: previousClose, low: previousClose, close: previousClose, volume: 0, lastTs: undefined});
            continue;
        }
...
const Chart: React.FC<ChartProps> = ({ data, supply }) => {
    ...
    const candlestickSeriesData: CandlestickData[] = useMemo(() => candles.map((candle) => ({time: candle.time as Time, open: candle.open, high: candle.high, low: candle.low, close: candle.close})), [candles]);
    ...
    useEffect(() => {
        ...
        const chart = createChart(container, {
            ...
            layout: { background: { color: '#0b0b0d' }, textColor: '#bdfbe2', attributionLogo: true, },
            grid: { vertLines: { color: 'rgba(180, 255, 231, 0.12)' }, horzLines: { color: 'rgba(180, 255, 231, 0.12)' }, },
            crosshair: {
                mode: CrosshairMode.Normal,
                vertLine: {color: 'rgba(0, 255, 194, 0.4)', width: 1, style: LineStyle.Solid, labelBackgroundColor: 'rgba(0, 255, 194, 0.1)',},
                horzLine: {color: 'rgba(0, 255, 194, 0.4)', width: 1, style: LineStyle.Solid, labelBackgroundColor: 'rgba(0, 255, 194, 0.1)',},
            },
            rightPriceScale: {borderVisible: false, autoScale: true, scaleMargins: {top: 0.1, bottom: 0.25},},
            timeScale: {timeVisible: true, secondsVisible: timeframe <= 15 * 60 * 1000, borderVisible: false,},
            localization: {
        ...
        const candleSeries = chart.addSeries(CandlestickSeries, {
            ...
            priceFormat: {type: 'price', precision: metric === 'mcap' ? 2 : 8, minMove: metric === 'mcap' ? 0.01 : 0.00000001,},
    ...
    useEffect(() => {
        const candleSeries = candleSeriesRef.current;
        if (!candleSeries) return;
        candleSeries.applyOptions({priceFormat: {type: 'price', precision: metric === 'mcap' ? 2 : 8, minMove: metric === 'mcap' ? 0.01 : 0.00000001,},});
    }, [metric]);
    useEffect(() => {
        if (!chartRef.current) return;
        const resize = () => {
    ...
    return (
        <div className={`${fullscreenClass} flex flex-col overflow-hidden border border-emerald-400/20 bg-[#0b0b0d] shadow-[0_0_35px_rgba(0,255,170,0.18)]`} style={containerStyle}>
            <div className="relative flex-1">
                ...
                <div ref={tooltipRef} className="pointer-events-none absolute hidden max-w-[220px] rounded-md border border-emerald-400/40 bg-[#050509]/95 p-3 text-xs text-white shadow-[0_0_20px_rgba(0,255,170,0.25)]" />
            </div>
            <div className="flex h-16 flex-wrap items-center justify-between gap-2 border-t border-white/5 bg-[#050509]/80 px-1 sm:px-3 py-2 mb-5 sm:mb-0">
                <div className="flex items-center sm:gap-2">
                    {TIMEFRAMES.map((option) => {
                        ...
                        return (<button key={option.label} type="button" onClick={() => handleTimeframeChange(option.value)} className={`rounded-md px-3 py-1.5 text-xs font-semibold transition ${isActive ? 'bg-emerald-400/20 text-emerald-200 shadow-[0_0_12px_rgba(0,255,170,0.35)]' : 'text-white/60 hover:text-emerald-200'}`}>{option.label}</button>);
                    })}
                </div>
                <div className="flex items-center gap-1">
                    {(['price', 'mcap'] as Metric[]).map((m) => {
                        ...
                        return (<button key={m} type="button" onClick={() => setMetric(m)} className={`rounded-md px-3 py-1.5 text-xs font-semibold transition ${active ? 'bg-emerald-400/20 text-emerald-200 shadow-[0_0_12px_rgba(0,255,170,0.35)]' : 'text-white/60 hover:text-emerald-200'}`} aria-pressed={active}>{label}</button>);
                    })}
                </div>
            </div>
        </div>
