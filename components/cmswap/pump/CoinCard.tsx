...
export type CoinCardData = {id: string; href: string; name: string; symbol: string; logoUrl: string; marketCapDisplay: string; createdAgo?: string; progressPercent?: number;};
export default function CoinCard({href, logoUrl, name, symbol, marketCapDisplay, createdAgo, progressPercent}: CoinCardData) {
	...
	return (
		<Link href={href} prefetch={false} className="group relative flex h-full flex-col gap-5 overflow-hidden rounded-lg bg-[#0a111f]/80 p-3 border border-white/5 shadow-lg shadow-black/30 transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02] hover:border hover:border-white">
			<div className="flex items-start gap-4">
				<div className="relative h-40 w-40 shrink-0 overflow-hidden rounded-2xl bg-white/10">
					<Image src={src} alt={`${name}`} onError={() => setSrc("/default.ico")} fill className="object-cover" sizes="256px" />
				</div>
					...
					<div className="mt-2">
						<div className="relative h-2 w-full overflow-hidden rounded-full">
							...
							<div className="relative h-2 overflow-hidden rounded-full bg-white/10">
								<div className="relative h-full rounded-full bg-gradient-to-r from-emerald-400 via-cyan-400 to-sky-500 animate-bonding" style={{ width: `${clampedProgress}%` }}>
