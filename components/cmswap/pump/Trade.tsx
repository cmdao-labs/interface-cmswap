...
type FiltersState = { time: "all" | "5m" | "1h" | "24h" | "7d"; from: string; actions: { buy: boolean; sell: boolean }; nativeMin: string; nativeMax: string; tokenMin: string; tokenMax: string; hash: string; };
type SocialField = "fb" | "x" | "telegram" | "website";
type SocialItem = { icon: React.ReactElement; field: SocialField; label: string; placeholder: string; };
type PresetButton = { label: string; type: "amount" | "percent"; value: number };
type Holder = { addr: string; value: number };
        ...
        <div className={containerClasses}>
            ...
            <div className="mt-6">
                <div>
                    ...
                    <div className="mt-2 flex w-full items-center justify-between overflow-hidden rounded-2xl border border-white/10 bg-white/10 px-4 py-3">
                        <input type="number" placeholder="0.0" value={inputBalance} onChange={(event) => onInputChange(event.target.value)} className="flex-1 bg-transparent text-2xl font-semibold text-white outline-none placeholder:text-white/30" />
                        <span className="-ml-10 text-sm uppercase tracking-wide text-white/60">{inputAssetSymbol}</span>
                    </div>
                    <div className="mt-2 flex flex-col items-start justify-between gap-2 text-xs text-white/50 xl:flex-row xl:items-center">
...
interface BondingStatusCardProps { className?: string; isGraduated: boolean; progressPercent: number; bondingTooltip: string; graduationLink: string; }
function BondingStatusCard({ className, isGraduated, progressPercent, bondingTooltip, graduationLink }: BondingStatusCardProps) {
    const containerClasses = ["rounded-2xl border border-white/10 bg-black/50 p-4 text-sm text-white/60", className].filter(Boolean).join(" ");
    return (
        <div className={containerClasses}>
            ...
            <div className="mt-4">
                ...
                <p className="mt-2 text-[11px] leading-relaxed text-white/45">
                    {bondingTooltip}
                    {isGraduated && graduationLink && (<>{" "}<Link href={graduationLink} prefetch={false} target="_blank" rel="noopener noreferrer" className="text-emerald-200 underline-offset-2 transition hover:text-emerald-100 hover:underline">View graduation txn</Link></>)}
                </p>
            </div>
        </div>
    );
}
interface HoldersCardProps { className?: string; sortedHolders: Holder[]; paginatedHolders: Holder[]; holdersPage: number; holdersTotalPages: number; rowsPerPage: number; onPrevPage: () => void; onNextPage: () => void; getExplorerAddressUrl: (address: string) => string; factoryAddr: string; creator: string | null; withChrome?: boolean; showHeader?: boolean; listClassName?: string; }
function HoldersCard({className, sortedHolders, paginatedHolders, holdersPage, holdersTotalPages, rowsPerPage, onPrevPage, onNextPage, getExplorerAddressUrl, factoryAddr, creator, withChrome = true, showHeader = true, listClassName}: HoldersCardProps) {
    const baseClasses = withChrome ? "rounded-3xl border border-white/10 bg-black/30 p-4 shadow-xl backdrop-blur" : "";
    const containerClasses = [baseClasses, className].filter(Boolean).join(" ");
    const creatorAddress = String(creator || "").toUpperCase();
    const defaultListClasses = withChrome ? "mt-4 space-y-3 overflow-y-auto pr-1" : "space-y-3 overflow-y-auto pr-1";
    const listClasses = [defaultListClasses, listClassName].filter(Boolean).join(" ");
    return (
        <div className={containerClasses}>
            {showHeader && <h2 className="text-lg font-semibold text-white">Holder</h2>}
            <div className={listClasses}>
                {sortedHolders.length === 0 ?
                    ...
                    paginatedHolders.map((res, index) => {
                        const addressLabel = res.addr.toUpperCase() === factoryAddr.toUpperCase() ? "Liquidity pool" : res.addr.toUpperCase() === creatorAddress ? "Creator" : `${res.addr.slice(0, 6)}...${res.addr.slice(-4)}`;
                        return (
                            <div key={`${res.addr}-${index}`} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/5 bg-white/5 p-4 text-sm text-white/80">
                                <div className="flex items-center gap-3">
                                    <span className="text-white/40">#{(holdersPage - 1) * rowsPerPage + index + 1}</span>
                                    <Link href={getExplorerAddressUrl(res.addr)} prefetch={false} target="_blank" rel="noopener noreferrer" className={`text-xs transition hover:text-white ${res.addr.toUpperCase() === creatorAddress || res.addr.toUpperCase() === factoryAddr.toUpperCase() ? "text-emerald-300" : "text-white/70"}`}>{addressLabel}</Link>
                                </div>
            ...
            {sortedHolders.length > 0 && (
                <div className="mt-3 flex items-center justify-between text-xs text-white/60">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={onPrevPage} disabled={holdersPage <= 1} className="rounded-md border border-white/10 bg-white/10 px-2 py-1 transition hover:border-white/20 hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-40">Previous</button>
                        <button onClick={onNextPage} disabled={holdersPage >= holdersTotalPages} className="rounded-md border border-white/10 bg-white/10 px-2 py-1 transition hover:border-white/20 hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-40">Next</button>
                    </div>
                </div>
            )}
        </div>
...
interface FiltersPopoverControlProps { buttonTitle: string; ariaLabel: string; hasActiveFilters: boolean; clearFilters: () => void; filters: FiltersState; handleTimeChange: (value: FiltersState["time"]) => void; handleTextChange: (field: "from" | "hash") => (event: React.ChangeEvent<HTMLInputElement>) => void; handleActionToggle: (key: "buy" | "sell") => void; handleNumberChange: (field: "nativeMin" | "nativeMax" | "tokenMin" | "tokenMax") => (event: React.ChangeEvent<HTMLInputElement>) => void; handleActionsReset: () => void; }
function FiltersPopoverControl({buttonTitle, ariaLabel, hasActiveFilters, clearFilters, filters, handleTimeChange, handleTextChange, handleActionToggle, handleNumberChange, handleActionsReset}: FiltersPopoverControlProps) {
    const buttonClasses = `rounded-md px-2 py-1 transition border ${hasActiveFilters ? "border-emerald-400/50 text-emerald-200" : "border-white/10 text-white/80 hover:border-white/20"}`;
    return (
        <Popover>
            <PopoverTrigger asChild>
                <button className={buttonClasses} title={buttonTitle} aria-label={ariaLabel}><FilterIcon size={14} /></button>
            </PopoverTrigger>
            <PopoverContent className="z-50 w-[320px] p-0">
                <div className="p-3 space-y-3 text-[11px] text-white/80">
                    ...
                    <div className="space-y-2">
                        ...
                        <select className="w-full rounded-md border border-white/10 bg-black/60 p-2 text-xs outline-none hover:border-white/20" value={filters.time} onChange={(e) => handleTimeChange(e.target.value as FiltersState["time"])}>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={handleActionsReset} className="rounded-full border px-2 py-1 text-[11px] border-white/10 text-white/60 hover:border-white/20" title="Show all">All</button>
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="block text-white/60">Native amount</label>
                        <div className="flex items-center gap-2">
                            <input type="number" inputMode="decimal" value={filters.nativeMin} onChange={handleNumberChange("nativeMin")} placeholder="min" className="w-full rounded-md border border-white/10 bg-black/60 px-2 py-2 text-xs placeholder-white/30 outline-none hover:border-white/20" />
                            <input type="number" inputMode="decimal" value={filters.nativeMax} onChange={handleNumberChange("nativeMax")} placeholder="max" className="w-full rounded-md border border-white/10 bg-black/60 px-2 py-2 text-xs placeholder-white/30 outline-none hover:border-white/20" />
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="block text-white/60">Token amount</label>
                        <div className="flex items-center gap-2">
                            <input type="number" inputMode="decimal" value={filters.tokenMin} onChange={handleNumberChange("tokenMin")} placeholder="min" className="w-full rounded-md border border-white/10 bg-black/60 px-2 py-2 text-xs placeholder-white/30 outline-none hover:border-white/20" />
                            <input type="number" inputMode="decimal" value={filters.tokenMax} onChange={handleNumberChange("tokenMax")} placeholder="max" className="w-full rounded-md border border-white/10 bg-black/60 px-2 py-2 text-xs placeholder-white/30 outline-none hover:border-white/20" />
                        </div>
                    </div>
                    <div className="space-y-2">
...
interface ProjectInfoSectionProps { className?: string; canEditSocials: boolean; onEditSocials: () => void; description: string | null; socialItems: SocialItem[]; socials: Record<SocialField, string>; ticker: string; onCopyAddress: (value: string) => void; copiedAddress: string | null; explorerUrl: string; variant?: "default" | "compact"; }
function ProjectInfoSection({className, canEditSocials, onEditSocials, description, socialItems, socials, ticker, onCopyAddress, copiedAddress, explorerUrl, variant = "default"}: ProjectInfoSectionProps) {
    const socialLinks = socialItems.filter((item) => socials[item.field]);
    const socialsSpacing = variant === "compact" ? "mt-4" : "mt-6";
    const tickerSpacing = variant === "compact" ? "mt-4" : "mt-6";
    return (
        <div className={className}>
            <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-lg font-semibold text-white">Project Info</h2>
                {canEditSocials && (<button onClick={onEditSocials} className="text-xs text-emerald-300 underline-offset-2 transition hover:text-emerald-100 hover:underline">Edit socials</button>)}
            </div>
            <p className="mt-4 text-sm leading-relaxed text-white/70">{description ? description : "No description has been shared yet."}</p>
            <div className={`${socialsSpacing} flex gap-3`}>
                {socialLinks.length === 0 ? 
                    <div className="rounded-2xl border border-dashed border-white/10 p-4 text-sm text-white/50">No socials linked yet.</div> : 
                    socialLinks.map((item) => (<Link key={item.field} href={socials[item.field]} prefetch={false} target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/10 bg-white/5 p-3 text-sm text-white/80 transition hover:border-white/30 hover:text-white">{item.icon}</Link>))
                }
            </div>
            <div className={`${tickerSpacing} rounded-2xl border border-white/10 bg-black/50 p-4 text-xs text-white/60`}>
                <div className="break-all text-[10px] sm:text-sm text-white/80">{ticker}</div>
                <div className="mt-3 flex flex-wrap gap-2">
                    <button onClick={() => onCopyAddress(ticker)} className="rounded-full border border-white/10 px-3 py-1 transition hover:border-white/30 hover:text-white">{copiedAddress === ticker ? "Copied" : "Copy"}</button>
                    <Link href={`${explorerUrl}address/${ticker}`} prefetch={false} target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/10 px-3 py-1 text-emerald-200 transition hover:border-emerald-200 hover:text-emerald-100">View on explorer</Link>
                </div>
            </div>
        </div>
    );
...
export default function Trade({ mode, chain, ticker, lp, token }: { mode: string; chain: string; ticker: string; lp: string; token: string; }) {
    let _chainId = 0; let _explorer = ""; let _rpc = "";
    ...
    React.useEffect(() => {if (headnoti) setHeadnotiShaken(false)}, [headnoti]);
    ...
    const { price24hAgo, changeAbs, changePct, athPrice } = React.useMemo(() => {
        ...
        if (Array.isArray(graphData) && graphData.length > 0) {
            ...
            for (const p of sorted) if (Number.isFinite(p.price)) ath = Math.max(ath, Number(p.price));
    ...
    const chainLabel = React.useMemo(() => {
        switch (chain) {
            case "kubtestnet": return "Bitkub Testnet";
            default: return chain ? chain.toUpperCase() : "Bitkub Testnet";
        }
    }, [chain]);
    const modeLabel = mode === "pro" ? "Pro Mode" : "Lite Mode";
    const presetButtons = React.useMemo(() => 
        mode === "pro" && trademode ? 
            [{ label: "0.1", type: "amount" as const, value: 0.1 }, { label: "0.5", type: "amount" as const, value: 0.5 }, { label: "1", type: "amount" as const, value: 1 },] : 
            [25, 50, 75].map((percent) => ({label: `${percent}%`, type: "percent" as const, value: percent}))
    , [mode, trademode]);
    const tokenSymbolDisplay = symbol ? String(symbol) : "";
    const inputAssetSymbol = trademode ? baseAssetSymbol : tokenSymbolDisplay.length >= 7 ? tokenSymbolDisplay.slice(0, 6) + "..." : tokenSymbolDisplay;
    const outputAssetSymbol = trademode ? tokenSymbolDisplay.length >= 7 ? tokenSymbolDisplay.slice(0, 6) + "..." : tokenSymbolDisplay : baseAssetSymbol;
    const formattedAvailableBalance = React.useMemo(() => 
        Intl.NumberFormat("en-US", {notation: "compact", compactDisplay: "short", maximumFractionDigits: 3}).format(
            mode === "pro" ? 
    ...
    const gradientButtonStyle = (active: boolean, variant: "buy" | "sell") => {
        ...
        const backgroundImage = variant === "buy" ? "radial-gradient(circle farthest-corner at 10% 20%, rgba(0,255,147,1) 0.2%, rgba(22,255,220,1) 100.3%)" : "linear-gradient(135deg, #F87171, #F43F5E)";
        return { backgroundImage } as React.CSSProperties;
    };
    const handlePresetClick = (preset: { label: string; type: "amount" | "percent"; value: number; }) => {
        if (preset.type === "amount") {const formatted = preset.value.toFixed(6); setInputBalance(formatted); qoute(formatted); return;}
        try {
            const balance: bigint = mode === "pro" ? 
                BigInt(state[1].result as bigint) : 
                BigInt(trademode ? (state[0].result as bigint) : (state[1].result as bigint)); 
    ...
    const handleReset = () => {setInputBalance(""); setOutputBalance("0");};
    const handleMaxClick = () => {
        try {
            let value = ''; 
            if (mode === "pro") {
                value = trademode ? String(Number(formatEther(ethBal)) - 0.00001) : formatEther(state[1].result as bigint);
            } else {
                value = formatEther(trademode ? (state[0].result as bigint) : (state[1].result as bigint));
            } 
            if (!Number.isFinite(Number(value))) value = '0'; 
            if (Number(value) < 0) value = '0'; setInputBalance(value); qoute(value);
        } catch (error) {console.error("Failed to apply max", error)}
    };
    const handleModeChange = React.useCallback((mode: boolean) => {setTrademode(mode); setInputBalance(""); setOutputBalance("0");}, []);
    const tradeButtonLabel = isWalletReady ? trademode ? `Buy ${tokenSymbolDisplay || "token"}` : `Sell ${tokenSymbolDisplay || "token"}` : "Connect wallet";
    ...
    const handleSave = async () => {
        ...
        if (hasError) return;
        await writeContract(config, {...socialContrct, functionName: "setSocialMedia", args: [ticker as "0xstring", socials.fb, socials.x, socials.telegram, socials.website]});
        setShowSocials(false);
    };
    const socialItems: SocialItem[] = [
        { icon: <FaFacebookF className="text-white" />, field: "fb", label: "Facebook", placeholder: "Facebook URL" },
        { icon: <BsTwitterX className="text-white" />, field: "x", label: "X (Twitter)", placeholder: "X (Twitter) URL" },
        { icon: <FaTelegramPlane className="text-white" />, field: "telegram", label: "Telegram", placeholder: "Telegram URL" },
        { icon: <FaGlobe className="text-white" />, field: "website", label: "Website", placeholder: "Website URL" },
    ];
    const socialsResult = useReadContracts({contracts: [{address: socialAddr as "0xstring", abi: SocialsABI, functionName: "socials", chainId: _chainId, args: [ticker as "0xstring"]}]});
    ...
    const handleTextChange = (field: "from" | "hash") => (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setFilters((prev) => {const next = { ...prev, [field]: value } as typeof filters; debouncedApplyFilters(next); return next;});
    };
    const handleNumberChange = (field: "nativeMin" | "nativeMax" | "tokenMin" | "tokenMax") => (e: React.ChangeEvent<HTMLInputElement>) => {
        const value = e.target.value;
        setFilters((prev) => {const next = { ...prev, [field]: value } as typeof filters; debouncedApplyFilters(next); return next;});
    };
    const clearFilters = () => {const base = createFiltersState(); setFilters(base); setAppliedFilters({ ...base });};
    const hasActiveFilters = React.useMemo(() => {return (filters.time !== "all" || filters.from.trim() !== "" || !filters.actions.buy || !filters.actions.sell || filters.nativeMin.trim() !== "" || filters.nativeMax.trim() !== "" || filters.tokenMin.trim() !== "" || filters.tokenMax.trim() !== "" || filters.hash.trim() !== "")}, [filters]);
    const filteredHx = React.useMemo(() => {
        ...
        const effective = {...f, time: filters.time, actions: filters.actions};
        ...
        return hx.filter((res: any) => {
            ...
            if (actionKey in effective.actions) {if (!effective.actions[actionKey]) return false}
            ...
            if (minNative !== undefined && !Number.isNaN(minNative)) {if (nVal === undefined || nVal < minNative) return false}
            if (maxNative !== undefined && !Number.isNaN(maxNative)) {if (nVal === undefined || nVal > maxNative) return false}
    ...
    const traderStats = React.useMemo(() => {
        ...
        filteredHx.forEach((res: any) => {
            ...
            if (!entry) {
                entry = {address: from, totalBought: 0, totalSold: 0, trades: 0, lastActive: 0};
                stats.set(key, entry);
            }
    ...
    const paginatedHolders = React.useMemo(() => {const start = (holdersPage - 1) * ROWS_PER_PAGE; return sortedHolders.slice(start, start + ROWS_PER_PAGE);}, [sortedHolders, holdersPage]);
    const tradersTotalPages = React.useMemo(() => Math.max(1, Math.ceil(traderStats.length / ROWS_PER_PAGE)), [traderStats.length]);
    const paginatedTraders = React.useMemo(() => {const start = (tradersPage - 1) * ROWS_PER_PAGE; return traderStats.slice(start, start + ROWS_PER_PAGE);}, [traderStats, tradersPage]);
    ...
    const handleHoldersPrev = React.useCallback(() => {setHoldersPage((p) => Math.max(1, p - 1))}, []);
    const handleHoldersNext = React.useCallback(() => {setHoldersPage((p) => Math.min(holdersTotalPages, p + 1))}, [holdersTotalPages]);
    const [copiedAddress, setCopiedAddress] = useState<string | null>(null);
    const copyToClipboard = async (address: string): Promise<void> => {await navigator.clipboard.writeText(address); setCopiedAddress(address); setTimeout(() => { setCopiedAddress(null) }, 2000);};
    React.useEffect(() => {
            ...
            try {
                ...
                try {
                    ...
                    if (hres.ok) {
                        ...
                        const holders = raw.map((h: any) => ({addr: String(h.holder || h.addr || ''), value: Number(h.balance || h.value || 0) / 1e25}))
                        setHolder(holders)
                    }
        ...
        return () => {clearInterval(summaryTimer); clearInterval(bodyTimer);};
    ...
    const handleInputChange = React.useCallback((value: string) => {setInputBalance(value); qoute(value);}, [qoute]);
    const trade = async () => {
        try {
            let result: any = "";
            if (chain === "kubtestnet") {
                if (trademode) {
                    ...
                    const allowance = await readContracts(config, {contracts: [{address: ticker as "0xstring", abi: erc20Abi, functionName: "allowance", args: [account.address as "0xstring", factoryContract.address as "0xstring"], chainId: _chainId}]});
                    if (Number(formatEther(allowance[0].result!)) < Number(inputBalance)) {
                        const { request } = await simulateContract(config, {address: ticker as "0xstring", abi: erc20Abi, functionName: "approve", args: [factoryContract.address as "0xstring", parseEther(String(Number(inputBalance) + 1))], chainId: _chainId});
    ...
    React.useEffect(() => {
        if (socialsResult.status === "success" && !hasSetSocialsRef.current) {
            ...
            } else {console.warn("Unexpected result format", rawResult)}
        }
    }, [socialsResult]);
        ...
        <main className="relative min-h-screen w-full 2xl:w-5/6 overflow-hidden pt-14 sm:pt-20 text-white">
            ...
            {headnoti && (
                <div className="fixed top-22 left-0 right-0 z-50 px-2 sm:px-4">
                    <div className={`mx-auto w-full 2xl:w-5/6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-emerald-500 bg-emerald-900 px-4 py-3 text-sm shadow-[0_0_25px_rgba(16,185,129,0.25)] ` + (!headnotiShaken && "animate-shake-once")} onAnimationEnd={(e) => {if (e.animationName === 'shake') setHeadnotiShaken(true)}}>
                        <div className="flex items-center gap-2 text-emerald-200"><Check size={16} /><span>Transaction sent</span></div>
                    ...
                    <div className="flex flex-1 flex-col gap-2 sm:gap-4">
                        ...
                        <div className="flex flex-wrap flex-row gap-1 sm:gap-2 text-[10px] sm:text-xs text-white/70">
                            <div className="flex flex-row items-center gap-2 rounded-full border border-white/10 bg-black/60 px-3 py-2">
                                <Sprout className="h-4 w-4" aria-hidden="true" /><p>{relativeCreatedTime && relativeCreatedTime}</p>
                            </div>
                            <div className="flex flex-row items-center gap-2 rounded-full border border-white/10 bg-black/60 px-3 py-2">
                                <Users className="h-4 w-4" aria-hidden="true" /><p>{holder.length}</p>
                            </div>
                            <div className="rounded-full border border-white/10 bg-black/60 px-3 py-1 sm:py-0 flex flow-col items-center gap-2">
                                ...
                                <button className="rounded-full border border-white/10 bg-white/10 p-1 transition hover:border-white/40 hover:bg-white/20" onClick={async () => {if (!ethereum) return; await ethereum.request({method: "wallet_watchAsset", params: { type: "ERC20", options: {address: ticker, symbol: tokenSymbolDisplay, decimals: 18, image: resolvedLogo} }});}} title="Add token to wallet"><Plus size={12} /></button>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[2fr_1fr] gap-4">
                <div className="space-y-6">
                    ...
                    <div className="sm:hidden rounded-3xl border border-white/10 bg-black/30 p-4 shadow-xl backdrop-blur">
                        ...
                        <TabsContent value="info">
                            <ProjectInfoSection className="mt-2" variant="compact" canEditSocials={creator === account.address} onEditSocials={() => setShowSocials(true)} description={description} socialItems={socialItems} socials={socials} ticker={ticker} onCopyAddress={copyToClipboard} copiedAddress={copiedAddress} explorerUrl={_explorer} />
                        </TabsContent>
                            <TabsContent value="traders">
                                <div className="ml-auto flex items-center gap-2 text-xs">
                                    <FiltersPopoverControl buttonTitle="Filter traders" ariaLabel="Filter traders" hasActiveFilters={hasActiveFilters} clearFilters={clearFilters} filters={filters} handleTimeChange={handleTimeChange} handleTextChange={handleTextChange} handleActionToggle={handleActionToggle} handleNumberChange={handleNumberChange} handleActionsReset={handleActionsReset} />
                                        ...
                                        <tbody>
                                            {paginatedTraders.length === 0 ? 
                                                ...
                                                (paginatedTraders.map((trader) => {
                                                    const profitClass = trader.profit > 0 ?  "text-emerald-300" : trader.profit < 0 ? "text-rose-300" : "text-white";
                                                    ...
                                                    const profitLabel = trader.profit > 0 ? `+${formattedProfit}` : trader.profit < 0 ? `-${formattedProfit}` : formattedProfit;
                                                    return (
                                                        <tr key={trader.address} className="text-xs text-white/80 hover:bg-white/10 border-t border-white/10">
                                                            <td className="py-6"><Link href={getExplorerAddressUrl(trader.address)} prefetch={false} target="_blank" rel="noopener noreferrer" className="text-white/70 underline-offset-2 transition hover:text-white hover:underline">{trader.address.slice(0, 6)}...{trader.address.slice(-4)}</Link></td>
                            ...
                            <TabsContent value="activity">
                                <div className="ml-auto flex items-center gap-2 text-xs">
                                    <FiltersPopoverControl buttonTitle="Filter activity" ariaLabel="Filter activity" hasActiveFilters={hasActiveFilters} clearFilters={clearFilters} filters={filters} handleTimeChange={handleTimeChange} handleTextChange={handleTextChange} handleActionToggle={handleActionToggle} handleNumberChange={handleNumberChange} handleActionsReset={handleActionsReset} />
                                            ...
                                            {filteredHx.length === 0 ? 
                                                ...
                                                paginatedActivity.map((res) => (
                                                    <tr key={res.hash} className="text-xs text-white/80 hover:bg-white/10 border-t border-white/10">
                                                        ...
                                                        <td className="py-6"><Link href={getExplorerAddressUrl(res.from)} prefetch={false} target="_blank" rel="noopener noreferrer" className="text-white/70 underline-offset-2 transition hover:text-white hover:underline">{res.from.slice(0, 6)}...{res.from.slice(-4)}</Link></td>
                                                        <td className="sm:px-0 py-6"><div className={`inline-block rounded-full bg-white/10 px-2 py-1 capitalize font-semibold uppercase ${res.action === "buy" ? "text-emerald-300" : res.action === "sell" ? "text-rose-300" : "text-cyan-300"}`}>{res.action}</div></td>
                                                        ...
                                                        <td className="py-6"><Link href={`${_explorer}tx/${res.hash}`} prefetch={false} target="_blank" rel="noopener noreferrer" className="text-emerald-200 underline-offset-2 transition hover:text-emerald-100 hover:underline">{res.hash.slice(0, 6)}...{res.hash.slice(-4)}</Link></td>
                                                    </tr>
                                                ))
                                            }
                                        </tbody>
                            ...
                            <TabsContent value="holders">
                                <HoldersCard className="mt-2" sortedHolders={sortedHolders} paginatedHolders={paginatedHolders} holdersPage={holdersPage} holdersTotalPages={holdersTotalPages} rowsPerPage={ROWS_PER_PAGE} onPrevPage={handleHoldersPrev} onNextPage={handleHoldersNext} getExplorerAddressUrl={getExplorerAddressUrl} factoryAddr={factoryAddr} creator={creator} withChrome={false} showHeader={false} />
                            </TabsContent>
                        </Tabs>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <HoldersCard className="hidden sm:block" sortedHolders={sortedHolders} paginatedHolders={paginatedHolders} holdersPage={holdersPage} holdersTotalPages={holdersTotalPages} rowsPerPage={ROWS_PER_PAGE} onPrevPage={handleHoldersPrev} onNextPage={handleHoldersNext} getExplorerAddressUrl={getExplorerAddressUrl} factoryAddr={factoryAddr} creator={creator} />
                </div>
            </div>
            {showSocials && (
            ...
            <style jsx>{`@keyframes fireShift { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } } @keyframes fireGlow { 0%, 100% { opacity: 0.35; filter: blur(6px); } 50% { opacity: 0.75; filter: blur(12px); } } @keyframes bondingShift { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } } @keyframes bondingGlow { 0%, 100% { opacity: 0.35; filter: blur(6px); } 50% { opacity: 0.75; filter: blur(12px); } } @keyframes spark { 0% { background-position: 0 0, 0 0; opacity: 0.25; } 50% { background-position: 14px 0, 18px 0; opacity: 0.6; } 100% { background-position: 0 0, 0 0; opacity: 0.25; } } .animate-fire { background-size: 200% 200%; animation: fireShift 2.2s ease-in-out infinite; } .animate-fire-glow { background-size: 200% 200%; animation: fireGlow 1.6s ease-in-out infinite, fireShift 2.8s linear infinite; } .animate-bonding { background-size: 200% 200%; animation: bondingShift 2.2s ease-in-out infinite; } .animate-bonding-glow { background-size: 200% 200%; animation: bondingGlow 1.6s ease-in-out infinite, bondingShift 2.8s linear infinite; } .animate-spark { animation: spark 0.9s linear infinite; }`}</style>
        </main>
    );
}
