...
const { tokens, POSITION_MANAGER, v3FactoryContract, positionManagerContract, erc20ABI, v3PoolABI, wrappedNative, } = chains[10143]
...
export default function Positions10143({ setIsLoading, setErrMsg, }: {
    ...
    const setAlignedAmountA = useDebouncedCallback(async (_amountB: string) => {
        ...
        const pool = new Pool(Token0, Token1, Number(feeSelect), sqrtPriceX96.toString(), liquidity.toString(), tick)
        if (String(token0).toUpperCase() === tokenAvalue.toUpperCase()) {
            const singleSidePositionToken0 = Position.fromAmount0({pool, tickLower: Number(lowerTick), tickUpper: Number(upperTick), amount0: String(parseUnits(_amountB, tokenB.decimal)) as BigintIsh, useFullPrecision: true})
            setAmountA(formatEther(singleSidePositionToken0.mintAmounts.amount1 as unknown as bigint))
        } else {
            const singleSidePositionToken1 = Position.fromAmount1({pool, tickLower: Number(lowerTick), tickUpper: Number(upperTick), amount1: String(parseUnits(_amountB, tokenB.decimal)) as BigintIsh})
    ...
    const clearState = () => {setTokenA(tokens[0]); setTokenB({name: 'Choose Token', value: '' as '0xstring', logo: '../favicon.ico', decimal: 18}); setFeeSelect(10000); setLowerTick(""); setUpperTick(""); setLowerPrice(""); setUpperPrice("");}
