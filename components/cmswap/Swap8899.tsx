...
import { useSwap8899PoolData } from '@/components/cmswap/swap/hooks/useSwap8899PoolData'
...
export default function Swap8899({ setIsLoading, setErrMsg, }: {
    ...
    const getQuote = useDebouncedCallback(async (_amount: string) => {
        ...
        if (wrappedRoute) {
            ...
            try {
                if (Number(_amount) !== 0) {
                        ...
                        if (useFunction) {
                            ...
                            if (poolSelect === "GameSwap") {
                                ...
                                if (amountA > 0 && amountB > 0) {
                                    const price = tokenAvalue.toUpperCase() === tokens[1].value.toUpperCase() ? amountB / amountA : amountA / amountB;
                    ...
                    if ((tokenAvalue.toUpperCase() === tokens[1].value.toUpperCase() && tokenBvalue.toUpperCase() === tokens[2].value.toUpperCase()) || (tokenAvalue.toUpperCase() === tokens[2].value.toUpperCase() && tokenB.value.toUpperCase() === tokens[1].value.toUpperCase())) {
                        ...
                        if (tokenAvalue.toUpperCase() === tokens[1].value.toUpperCase()) {
                            useFunction = 'getExpectedTokenFromJBC';
                        } else if (tokenAvalue.toUpperCase() === tokens[2].value.toUpperCase()) {
                            useFunction = 'getExpectedJBCFromToken';
                        }
                        if (useFunction) {
                            const quoteOutput = await readContracts(config, {contracts: [{...CMswapPoolDualRouterContract, functionName: useFunction, args: [poolAddr, parseUnits(_amount, getDecimals(tokenA))]}]});
                            const result = quoteOutput[0].result !== undefined ? quoteOutput[0].result : BigInt(0)
            ...
            } catch (error) {console.error("Error in getting GameSwap quote:", error)}
            //**---------- Jibswap */
            try {
                if (Number(_amount) !== 0) {
            ...
            } catch (error) {console.error("Error in getting JibSwap quote:", error)}
        }
    ...
    const handleSwap = async () => {
        ...
        else if (poolSelect === "GameSwap") gameswap();
        else if (poolSelect === "JibSwap") jibswap();
    }
    const wrap = async () => {
        setIsLoading(true)
        try {
        ...
        try {
            ...
            if (tokenAAddr !== tokens[1].value.toUpperCase()) {
                ...
                if (allowanceA < parsedAmountA) {
                    const { request } = await simulateContract(config, {...erc20ABI, address: tokenA.value as '0xstring', functionName: 'approve', args: [CMswapPoolDualRouter, parsedAmountA]});
            ...
            if (useFunction === 'swapCMJtoJBC' || useFunction === 'swapJUSDTtoJBC') {
                expectedOut = await readContract(config, {...CMswapPoolDualRouterContract, functionName: 'getExpectedJBCFromToken', args: [poolAddr, parsedAmountA]});
            } else if (useFunction === 'swapJC' || useFunction === 'swapJU') {
                expectedOut = await readContract(config, {...CMswapPoolDualRouterContract, functionName: 'getExpectedTokenFromJBC', args: [poolAddr, parsedAmountA]});
            } else {
            ...
            if (useFunction === 'swapJC' || useFunction === 'swapJU') {
                const { request } = await simulateContract(config, {...CMswapPoolDualRouterContract, functionName: useFunction, args: [minOut], value: parsedAmountA})
                const tx = await writeContract(config, request)
                await waitForTransactionReceipt(config, { hash: tx });
                setTxupdate(tx)
            } else {
                const { request } = await simulateContract(config, {...CMswapPoolDualRouterContract, functionName: useFunction, args: [parsedAmountA, minOut]})
                const tx = await writeContract(config, request)
                await waitForTransactionReceipt(config, { hash: tx });
                setTxupdate(tx)
            }
    ...
    React.useEffect(() => {if (poolSelect === "" && bestPool) setPoolSelect(bestPool);}, [bestPool]);
