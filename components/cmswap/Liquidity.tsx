...
type LiquidityRangeSliderProps = { currentPrice: number | null; lowerPercent: number | null; upperPercent: number | null; lowerPrice: number | null; upperPrice: number | null; zoomSpan: number; onLowerChange: (percent: number) => void; onUpperChange: (percent: number) => void; onLowerCommit: (percent: number) => void; onUpperCommit: (percent: number) => void; disabled?: boolean; upperIsInfinite?: boolean; distributionBuckets?: { ratio: number; height: number }[]; }
const LiquidityRangeSlider: React.FC<LiquidityRangeSliderProps> = ({ currentPrice, lowerPercent, upperPercent, lowerPrice, upperPrice, zoomSpan, onLowerChange, onUpperChange, onLowerCommit, onUpperCommit, disabled = false, upperIsInfinite = false, distributionBuckets }) => {
    ...
    React.useEffect(() => {if (!isDragging) setDraftLower(baseLower)}, [baseLower, isDragging])
    React.useEffect(() => {if (!isDragging) setDraftUpper(baseUpper)}, [baseUpper, isDragging])
    const draftLowerRef = React.useRef(draftLower)
    const draftUpperRef = React.useRef(draftUpper)
    React.useEffect(() => {draftLowerRef.current = draftLower}, [draftLower])
    React.useEffect(() => {draftUpperRef.current = draftUpper}, [draftUpper])
    ...
    const distribution = React.useMemo<DistributionBar[]>(() => {
        const bucketCount = (Array.isArray(distributionBuckets) && distributionBuckets.length > 0) ? distributionBuckets.length : 41
        ...
        <div className="space-y-4 rounded-xl border border-[#00ff9d]/15 bg-[#07111f]/80 p-4">
            <div className="relative h-16">
                <div ref={trackRef} className="absolute inset-x-4 top-1/2 h-2 -translate-y-1/2 rounded-full bg-[#0f1d2d] shadow-inner">
                    <div className="absolute inset-y-[-8px] w-[2px] rounded-full bg-emerald-400/70 shadow-[0_0_12px_rgba(16,185,129,0.45)]" style={{ left: `${currentRatio * 100}%`, transform: 'translateX(-50%)' }} />
                    <div className="absolute inset-y-[-4px] rounded-full border border-emerald-400/40 bg-emerald-500/20 backdrop-blur-sm" style={{ left: `${selectionStart * 100}%`, width: `${selectionWidth}%` }} />
                </div>
                <button type="button" role="slider" aria-valuemin={Math.max(windowMin, SLIDER_MIN_PERCENT)} aria-valuemax={draftUpper - SLIDER_MIN_GAP} aria-valuenow={draftLower} aria-label={`Lower range ${lowerPercentLabel}`} aria-disabled={disabled} disabled={disabled} onPointerDown={beginDrag('lower')} onKeyDown={handleKeyDown('lower')} className="absolute top-1/2 flex h-6 w-6 -translate-y-1/2 -translate-x-1/2 items-center justify-center rounded-full border border-emerald-400/70 bg-[#0a1a29] text-white shadow-[0_4px_16px_rgba(16,185,129,0.35)] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/70" style={{ left: `${toRatio(draftLower) * 100}%` }}>
                    <span className="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-emerald-500/20 px-2 py-[2px] text-[11px] font-medium text-emerald-100 shadow">{lowerPercentLabel}</span>
                    <span className="pointer-events-none absolute top-[110%] left-1/2 -translate-x-1/2 text-[10px] text-white/50">{lowerPriceLabel}</span>
                </button>
                <button type="button" role="slider" aria-valuemin={draftLower + SLIDER_MIN_GAP} aria-valuemax={windowMax} aria-valuenow={upperIsInfinite ? windowMax : draftUpper} aria-label={`Upper range ${upperPercentLabel}`} aria-disabled={disabled} disabled={disabled} onPointerDown={beginDrag('upper')} onKeyDown={handleKeyDown('upper')} className="absolute top-1/2 flex h-6 w-6 -translate-y-1/2 -translate-x-1/2 items-center justify-center rounded-full border border-emerald-400/70 bg-[#0a1a29] text-white shadow-[0_4px_16px_rgba(16,185,129,0.35)] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/70" style={{ left: `${toRatio(draftUpper) * 100}%` }}>
                    <span className="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-emerald-500/20 px-2 py-[2px] text-[11px] font-medium text-emerald-100 shadow">{upperPercentLabel}</span>
                    <span className="pointer-events-none absolute top-[110%] left-1/2 -translate-x-1/2 text-[10px] text-white/50">{upperPriceLabel}</span>
                </button>
                {disabled && <div className="pointer-events-none absolute inset-0 flex items-center justify-center rounded-xl bg-black/40 text-xs font-medium text-white/60">Select a token pair to unlock the range slider</div>}
            </div>
                <div className="space-y-2">
                    <div className="relative h-28 rounded-xl border border-white/10 bg-[#0d1b2b]/70">
                        ...
                        {distribution.map((bucket: DistributionBar) => (<div key={bucket.key} className={`flex-1 rounded-t transition-all ${bucket.inRange ? 'bg-emerald-400/60 shadow-[0_0_10px_rgba(16,185,129,0.25)]' : 'bg-white/15'} ${bucket.isCurrent ? 'ring-1 ring-emerald-300/60' : ''}`} style={{ height: `${bucket.height}%` }} />))}
                    </div>
                    <div className="pointer-events-none absolute inset-y-2 w-[2px] rounded-full bg-emerald-300/80 shadow-[0_0_10px_rgba(16,185,129,0.3)]" style={{ left: `${currentRatio * 100}%`, transform: 'translateX(-50%)' }} />
                    <div className="pointer-events-none absolute inset-y-0 rounded-xl bg-emerald-500/10" style={{ left: `${selectionStart * 100}%`, width: `${selectionWidth}%` }} />
                    {disabled && <div className="pointer-events-none absolute inset-0 flex items-center justify-center rounded-xl bg-black/40 text-[10px] font-medium text-white/60">Select tokens to view liquidity distribution</div>}
                </div>
            </div>
...
export default function LiquidityUnified({ setIsLoading, setErrMsg, }: {
    ...
    const sliderSpan = React.useMemo(() => SLIDER_ZOOM_SPANS[Math.min(sliderZoomIndex, SLIDER_ZOOM_SPANS.length - 1)], [sliderZoomIndex],)
    ...
    const sliderEnabled = Boolean(currentPriceNumber && tickSpacingNumber && tokenA.value !== ('0x' as '0xstring') && tokenB.value !== ('0x' as '0xstring'))
    ...
    React.useEffect(() => {fetchDistribution()}, [fetchDistribution])
    React.useEffect(() => {
    ...
    React.useEffect(() => {setSliderZoomIndex(2)}, [tokenA.value, tokenB.value])
    ...
    const placeLiquidity = async () => {
        ...
        try {
            ...
            if (pairDetect === '0x0000000000000000000000000000000000000000') {
                const { request: request0 } = await simulateContract(config, {...v3FactoryContract, functionName: 'createPool', args: [tokenAvalue as '0xstring', tokenBvalue as '0xstring', feeSelect]})
            ...
            const { request } = await simulateContract(config, {
                ...
                value: token0check.toUpperCase() === tokens[0].value.toUpperCase() ? parseUnits(amount0 || '0', decimal0) : (token1check.toUpperCase() === tokens[0].value.toUpperCase() ? parseUnits(amount1 || '0', decimal1) : BigInt(0))
            })
            const h = await writeContract(config, request)
            await waitForTransactionReceipt(config, { hash: h })
            setTxupdate(h)
    ...
    React.useEffect(() => {
        const fetch1 = async () => {
            if (tokenA.value.toUpperCase() === tokenB.value.toUpperCase()) setTokenB({ name: 'Choose Token', value: '0x' as '0xstring', logo: '../favicon.ico', decimal: 18 });
            const tokenAvalue = toWrapped(tokenA.value)
            const tokenBvalue = toWrapped(tokenB.value)
            ...
            if (stateB[2].result !== undefined && stateB[2].result !== '0x0000000000000000000000000000000000000000') {
                ...
                const _currPrice = token0.toUpperCase() === tokenBvalue.toUpperCase() ? ((Number(sqrtPriceX96) / (2 ** 96)) ** 2) * priceFactor : (1 / (((Number(sqrtPriceX96) / (2 ** 96)) ** 2) * priceFactor))
                ...
                const displayPriceFromTick = (t: number) => (token0.toUpperCase() === tokenBvalue.toUpperCase() ? priceAtTickBase(t) * priceFactor : (1 / priceAtTickBase(t)) * priceFactor)
    ...
    }, [address, tokenA, tokenB, feeSelect, rangePercentage, txupdate, POSITION_MANAGER, v3FactoryContract, v3PoolABI, erc20ABI])
    ...
    return (
        <div className='space-y-2'>
                            ...
                            <Button variant="ghost" role="combobox" aria-expanded={open} className="w-[180px] h-8 text-white hover:bg-[#162638] hover:text-[#00ff9d] flex items-center justify-between h-10 cursor-pointer">
                                <div className='gap-2 flex flex-row items-center justify-center'>
                                    <div className="w-5 h-5 rounded-full bg-[#00ff9d]/20">
                                        <span className="text-[#00ff9d] text-xs">{(tokenA.logo && tokenA.logo.trim() !== '' && tokenA.logo !== '../favicon.ico') ? <img alt="" src={tokenA.logo} className="size-5 shrink-0 rounded-full" /> : '?'}</span>
                                    </div>
                                        ...
                                        {tokens.map(token => (
                                            ...
                                            >
                                                <div className="flex items-center">
                                                    {((token as any).logo && String((token as any).logo).trim() !== '') ? <img alt="" src={(token as any).logo} className="size-5 shrink-0 rounded-full" /> : <div className="size-5 shrink-0 rounded-full bg-[#00ff9d]/10 text-center leading-5">?</div>}
                ...
                <div className="flex items-center justify-between">
                    ...
                    <Popover open={open2} onOpenChange={setOpen2}>
                        ...
                        <Button variant="ghost" role="combobox" aria-expanded={open} className="w-[180px] h-8 text-white hover:bg-[#162638] hover:text-[#00ff9d] flex items-center justify-between h-10 cursor-pointer">
                            <div className='gap-2 flex flex-row items-center justify-center'>
                                <div className="w-5 h-5 rounded-full bg-[#00ff9d]/20">
                                    <span className="text-[#00ff9d] text-xs">{(tokenB.logo && tokenB.logo.trim() !== '' && tokenB.logo !== '../favicon.ico') ? <img alt="" src={tokenB.logo} className="size-5 shrink-0 rounded-full" /> : '?'}</span>
                                </div>
            ...
            <div className="mt-4">
                <LiquidityRangeSlider currentPrice={currentPriceNumber} lowerPercent={lowerPercentValue} upperPercent={upperPercentValue} lowerPrice={lowerPriceNumber} upperPrice={upperPriceNumber} zoomSpan={sliderSpan} onLowerChange={handleSliderLowerChange} onUpperChange={handleSliderUpperChange} onLowerCommit={handleSliderLowerCommit} onUpperCommit={handleSliderUpperCommit} disabled={!sliderEnabled} upperIsInfinite={upperIsInfinite} distributionBuckets={distributionBuckets ?? undefined} />
            </div>
