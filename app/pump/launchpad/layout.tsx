...
export default function BlogLayout({
    ...
    const handleChain = (term: string) => {
        redirect('/pump/launchpad?chain=' + term);
    };
    ...
    return (
        <div className="grid grid-rows-[20px_1fr_10px] items-center justify-items-center min-h-screen pb-20 gap-16 p-4 sm:p-10 font-[family-name:var(--font-geist-sans)] bg-slate-900 text-white">
            <header className="mt-[150px] row-start-1 flex gap-4 md:gap-8 items-center justify-start md:justify-center mt-20 mb-10 text-xs md:text-sm w-full flex-wrap" style={{zIndex: 1}}>
                <div className="flex flex-row gap-1">
                    <button className="text-white hover:bg-neutral-800 focus:outline-none rounded-lg p-2" onClick={() => {if (chain !== 'unichain') {handleChain('unichain');}}}><Image src="/unichainMainnet.png" alt="unichain" width={25} height={25} style={{filter: chain !== 'unichain' ? "grayscale(1)" : "grayscale(0)"}} /></button>
                    <button className="text-white hover:bg-neutral-800 focus:outline-none rounded-lg p-2" onClick={() => {if (chain !== 'base') {handleChain('base');}}}><Image src="/baseMainnet.png" alt="unichain" width={25} height={25} style={{filter: chain !== 'base' ? "grayscale(1)" : "grayscale(0)"}} /></button>
                </div>
                ...
                {mode !== 'pro' && <Link href="https://forms.gle/R2iBpUXM8mtkxVEm6" rel="noopener noreferrer" target="_blank" prefetch={false} className="underline text-emerald-300 hover:font-bold">Faucet</Link>}
            </header>
            <div className="w-[100px] h-[100px] sm:w-[500px] sm:h-[500px] absolute top-0 right-0" style={{backgroundImage: mode === 'pro' ? 'radial-gradient(circle farthest-side at 100% 10%,rgb(110 231 183),#8586ad00 58%)' : 'radial-gradient(circle farthest-side at 100% 10%,#8586ad26,#8586ad00 88%)'}}></div>
