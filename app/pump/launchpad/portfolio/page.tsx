...
import Dashboard from "@/components/cmswap/pump/Dashboard";
import Activity from "@/components/cmswap/pump/Activity";
import Sort3 from "@/components/cmswap/pump/Sort3";

const chainLabelMap: Record<string, string> = { kubtestnet: "Bitkub Testnet", };
    ...
    <div className="rounded-3xl border border-white/10 bg-black/30 p-4 sm:p-6 shadow-xl backdrop-blur">
        <div className="space-y-4">
            ...
            <div className="space-y-3">
                {Array.from({ length: 5 }).map((_, index) => (<div key={index} className="h-12 w-full animate-pulse rounded-2xl bg-white/5" />))}
            </div>
        </div>
    </div>
);

export const metadata: Metadata = {title: "Portfolio | CMswap - PUMP", description: "hello pump.",};
export default async function Portfolio(props: {searchParams?: Promise<{ addr?: string; activity?: string; mode?: string; chain?: string; token?: string; }>;}) {
            ...
            <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-6 pt-2 pb-4">
                <header className="flex flex-col gap-6">
                    <div className="flex flex-wrap items-center justify-between gap-4 text-[10px] sm:text-xs uppercase tracking-[0.2em] text-white/60">
                        <Link href={backHref} prefetch={false} className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-white transition hover:border-white/30 hover:bg-white/10">
                            <ArrowLeft className="h-4 w-4" aria-hidden="true" />
                        </Link>
                        <div className="flex flex-wrap items-center gap-2">
                            {chainLabel && <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-white/70">{chainLabel}</span>}
                ...
                <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-black/40 p-4 shadow-[0_30px_120px_rgba(0,0,0,0.35)] backdrop-blur">
                    ...
                    <div className="relative z-10 flex flex-col gap-6">
                        ...
                        <Suspense key={`${addr}-${activity}-${mode}-${chain}-${token}`} fallback={<PortfolioSkeleton />}>
                            <div className="rounded-3xl border border-white/10 bg-black/30 p-4 sm:p-6 shadow-xl backdrop-blur">
                                {showActivity ? <Activity addr={addr} mode={mode} chain={chain} token={token} /> : <Dashboard addr={addr} mode={mode} chain={chain} token={token} />}
                            </div>
                        </Suspense>
                    </div>
                </section>
