...
export default function Sort4() {
    ...
    return (
        <div className="w-full xl:w-1/3 self-center bg-neutral-900 p-2 rounded-2xl flex flex-row justify-around border-solid border-2 border-emerald-300" style={{zIndex: 1}}>
            <span className={(mode === 'lite' || mode === undefined ? "text-black font-bold p-2 w-1/2 bg-black text-center rounded-lg" : "text-gray-400 underline cursor-pointer hover:font-bold p-2 w-1/2 text-center")} style={{backgroundImage: mode === 'lite' || mode === undefined ? 'radial-gradient( circle farthest-corner at 10% 20%,  rgba(0,255,147,1) 0.2%, rgba(22,255,220,1) 100.3% )' : 'none'}} onClick={() => handleMode('lite')}>Lite mode</span>
