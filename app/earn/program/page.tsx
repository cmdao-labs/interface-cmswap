...
/**
 * AdvancedStakingPlatform.tsx
 * Full page component for DeFi staking UI mockup
 */
...
export default function AdvancedStakingPlatform() {
  ...
  const renderStatsCard = (icon: React.ReactNode, title: string, value: string, change?: string, color?: string) => (
    <div className="bg-gradient-to-br from-gray-800/50 to-gray-700/30 backdrop-blur-sm border border-gray-600/20 rounded-xl p-4 hover:border-blue-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/10">
      <div className="flex items-center justify-between mb-2">
        <div className={`p-2 rounded-lg ${color || "bg-blue-500/20 text-blue-400"}`}>
          {icon}
        </div>
      {/*   {change && (
  ...
  const renderActionButton = (icon: React.ReactNode, label: string, onClick: () => void, variant: "default" | "outline" | "destructive" = "default") => (
    <Button
      className={`flex flex-wrap h-12 font-semibold transition-all duration-300 hover:scale-[1.02]
font-mono h-auto rounded text-xs   bg-[#162638] text-[#00ff9d] border border-[#00ff9d]/20        "border-gray-600 hover:border-blue-500/50 hover:bg-blue-500/10"
      }`}
      variant={variant}
      onClick={onClick}
    >
...
{/*                 <TableCell>
                  <Badge className="bg-green-500/20 text-green-400 border-green-500/30">{row.apy}</Badge>
                </TableCell> */}
                <TableCell>
                  <Badge variant="secondary" className="bg-blue-500/20 text-blue-400 border-blue-500/30">{row.status}</Badge>
                </TableCell>
                <TableCell>
          ...
          <TableBody>
            {lockHistory.map((row, i) => (
              <TableRow key={i} className="border-gray-700 hover:bg-gray-800/30">
                ...
                <TableCell className="font-semibold">{row.amount}</TableCell>
                {/* <TableCell>
                  <Badge className="bg-purple-500/20 text-purple-400 border-purple-500/30">{row.multiplier}</Badge>
                </TableCell> */}
                <TableCell className="font-mono text-sm">{row.unlockAt}</TableCell>
                <TableCell>
                  <Badge variant="secondary" className="bg-orange-500/20 text-orange-400 border-orange-500/30">{row.status}</Badge>
                </TableCell>
                <TableCell>
          ...
          <TableBody>
            {multipleLockHistory.map((row, i) => (
              <TableRow key={i} className="border-gray-700 hover:bg-gray-800/30">
                ...
                <TableCell className="font-semibold">{row.amount}</TableCell>
                <TableCell>
                  <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/30">{row.power}</Badge>
                </TableCell>
                <TableCell className="font-mono text-sm">{row.unlockAt}</TableCell>
                <TableCell>
                  <Badge variant="secondary" className="bg-orange-500/20 text-orange-400 border-orange-500/30">{row.status}</Badge>
          ...
          <TableBody>
            {nftHistory.map((row, i) => (
              <TableRow key={i} className="border-gray-700 hover:bg-gray-800/30">
                ...
                <TableCell>
                  <Badge className="bg-cyan-500/20 text-cyan-400 border-cyan-500/30">{row.fee}</Badge>
                </TableCell>
                  ...
                  {lockOptions.map((option) => (
                    <div
                      ...
                      className={`p-3 rounded-lg border cursor-pointer transition-all ${
                        selectedLockDuration === option.days
                    ...
                    >
                      {lockOptions.map((option) => (
                        <option key={option.days} value={option.days.toString()}>
                          {option.days} Days - {option.apy} APY ({option.multiplier}x)
                        </option>
                      ))}
                    </select>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white font-mono">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        ...
        <Link
          ...
          prefetch={false}
          className="underline hover:font-bold pl-4 xl:ml-4 pb-[6px]"
        >
            ...
            <CardTitle className="text-2xl font-bold flex items-center gap-2">
              ...
              <Dialog open={isExtendDialogOpen} onOpenChange={setIsExtendDialogOpen}>
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm" className="border-green-600 hover:border-green-500 hover:bg-green-500/10 text-green-400">
                ...
                <DialogContent className="bg-gray-900 border-gray-700 text-white">
                  <DialogHeader>
                    <DialogTitle className="text-green-400">Extend Staking Program</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-4 py-4">
                    <div>
              ...
              <Dialog open={isCancelDialogOpen} onOpenChange={setIsCancelDialogOpen}>
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm" className="border-red-600 hover:border-red-500 hover:bg-red-500/10 text-red-400">
                ...
                <DialogContent className="bg-gray-900 border-gray-700 text-white">
                  <DialogHeader>
                    <DialogTitle className="text-red-400">Cancel Staking Program</DialogTitle>
                  </DialogHeader>
                  <div className="py-4">
        ...
        <div className="flex flex-wrap gap-3">
          <Button
            variant={mode === "staking" ? "default" : "outline"}
            onClick={() => setMode("staking")}
            className={mode === "staking" ? "bg-gradient-to-r from-blue-500 to-purple-600 shadow-lg shadow-blue-500/25" : "border-gray-600 hover:border-blue-500/50 hover:bg-blue-500/10"}
          >
            <Coins className="w-4 h-4 mr-2" />
            Flexible Staking
          </Button>
          <Button
            variant={mode === "lock" ? "default" : "outline"}
            onClick={() => setMode("lock")}
            className={mode === "lock" ? "bg-gradient-to-r from-orange-500 to-red-600 shadow-lg shadow-orange-500/25" : "border-gray-600 hover:border-orange-500/50 hover:bg-orange-500/10"}
          >
          ...
          <Button
            ...
            className={mode === "multiple" ? "bg-gradient-to-r from-purple-500 to-pink-600 shadow-lg shadow-purple-500/25" : "border-gray-600 hover:border-purple-500/50 hover:bg-purple-500/10"}
          >
            <Zap className="w-4 h-4 mr-2" />
            Multiple Lock
          </Button>
          <Button
            variant={mode === "nft" ? "default" : "outline"}
            onClick={() => setMode("nft")}
            className={mode === "nft" ? "bg-gradient-to-r from-cyan-500 to-blue-600 shadow-lg shadow-cyan-500/25" : "border-gray-600 hover:border-cyan-500/50 hover:bg-cyan-500/10"}
          >
        ...
        <div className="bg-gradient-to-br from-gray-800/50 to-gray-700/30 backdrop-blur-sm rounded-xl p-6 border border-gray-600/20 mt-4">
          ...
          {/*     <button
                ...
                className={`px-3 py-2 rounded-md font-medium transition ${
                  activeTab === "overview" ? "bg-blue-600 text-white" : "text-gray-300 hover:text-white"
                }`}
              >
                Overview
              </button> */}
              <button
                ...
                className={`px-3 py-2 rounded-md font-medium transition ${
                  activeTab === "history" ? "bg-blue-600 text-white" : "text-gray-300 hover:text-white"
                }`}
              >
                Transaction History
              </button>
