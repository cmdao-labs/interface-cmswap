...
const CreateEarnProgram = () => {
    ...
    const [startDate, setStartDate] = useState<{ timestamp: number; formatted: string; } | null>(null);
    ...
    const fetchTokenData = async (
        ...
        try {
            ...
            if (nameResult === undefined || symbolResult === undefined || totalSupplyResult === undefined) {
            ...
            try {
                ...
                if (lpResult[0].status === 'success' && lpResult[1].status === 'success' && lpResult[0].result && lpResult[1].result) {
                    ...
                    if (tokenSymbols[0].status === 'success' && tokenSymbols[1].status === 'success' && tokenSymbols[0].result && tokenSymbols[1].result) {
                        ...
                        setInfo({name: combinedName, symbol: combinedSymbol, totalSupply});
                        return;
                    }
                }
            } catch {}
            setInfo({name, symbol, totalSupply});
        } catch (error) {
    ...
    useEffect(() => {
        async function fetchCurrentBlock() {
            try {
                ...
                if (!block) return;
                setBlocks(prev => [{blockNumber: Number(block.number), blockTimestamp: Number(block.timestamp)}, ...prev]);
            } catch (e) {
    ...
    useEffect(() => {
        if (rewardToken) {
            ...
            readContracts(config, {contracts: [{ abi: erc20Abi, address: rewardToken as `0x${string}`, functionName: 'symbol' }]})
                .then(([result]) => {
    ...
    useEffect(() => {
        if (openReward && commandInputRef.current) commandInputRef.current.focus();
    ...
    const estimatedStartBlock = estimateBlockNumberByTimestamp(startDate?.timestamp || 0, blocks.length > 0 ? blocks[0].blockNumber : 0, blocks.length > 0 ? blocks[0].blockTimestamp : 0, avgBlockTime);

    const createPool = async () => {
        try {
            if (stakingType === 'Token Staking') {
                const allowance = await readContract(config, {abi: erc20Abi, address: rewardToken as `0x${string}`, functionName: 'allowance', args: [address as `0x${string}`, lib.StakingFactoryV2_Addr]});
                if (allowance < parseEther(rewardAmount)) {
                    const { request } = await simulateContract(config, {abi: erc20Abi, address: rewardToken as `0x${string}`, functionName: 'approve', args: [lib.StakingFactoryV2_Addr, parseEther(rewardAmount)]});
                    const h = await writeContract(config, request);
                    await waitForTransactionReceipt(config, { hash: h });
                }
                const programName = selectLockOption === '1' ?
                    `Fixable Lock ${selectTokenInfo?.name} earn ${rewardTokenInfo?.name}` :
                    selectLockOption === '2' ? `Multiple Lock ${selectTokenInfo?.name} earn ${rewardTokenInfo?.name}` : `Staking ${selectTokenInfo?.name} earn ${rewardTokenInfo?.name}`;
                    ...
                    args: [
                        {
                            ...
                            lockDurations: selectLockOption === '1' ?
                                ...
                                selectLockOption === '2' ? multiLocksTime : [],
                ...
                const allowance = await readContract(config, {abi: erc20Abi, address: rewardToken as `0x${string}`, functionName: 'allowance', args: [address as `0x${string}`, lib.StakingFactoryV3_Addr]});
                if (allowance < parseEther(rewardAmount)) {
                    const { request } = await simulateContract(config, {abi: erc20Abi, address: rewardToken as `0x${string}`, functionName: 'approve', args: [lib.StakingFactoryV3_Addr, parseEther(rewardAmount)]});
                    const h = await writeContract(config, request);
                    await waitForTransactionReceipt(config, { hash: h });
                }
                const programName =`Staking Concentrate LP ${selectedPool?.name} earn ${rewardTokenInfo?.name}`
                if (selectedPool === null || selectedPool === undefined) return;
                const { request } = await simulateContract(config, {
    ...
    const renderStepIndicator = () => (
        ...
        {[
            ...
            <div key={step.number} className="flex items-center">
                <div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 ${currentStep >= step.number ? `${currentTheme.accent} ${currentTheme.bg}` : ` ${currentTheme.border} ${currentTheme.hover}`}`}>
                    {currentStep > step.number ? 
                        <Check className={`w-5 h-5 ${currentTheme.text}`} /> :
                        <span className={`text-sm font-medium ${currentStep >= step.number ? currentTheme.text : 'text-gray-400'}`}>{step.number}</span>
                    }
                </div>
                {index < arr.length - 1 && <div className={`w-16 h-0.5 mx-4 ${currentStep > step.number ? currentTheme.accent : 'bg-gray-600'}`} />}
            </div>
        ))}
        </div>
    );
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-3">
                                    {stakingTypes.map((type) => (
                                        <div key={type.name} onClick={() => setStakingType(type.name)} className={`p-4 border rounded-lg cursor-pointer transition-all ${stakingType === type.name ? `${currentTheme.accent} ${currentTheme.bg}` : `border ${currentTheme.border} ${currentTheme.border} `}`}>
                                ...
                                <button onClick={() => stakingType && setCurrentStep(2)} disabled={!stakingType} className={`w-full py-3 rounded-lg font-medium transition-all ${stakingType ? `${currentTheme.text} ${currentTheme.bg} border ${currentTheme.accent} hover:opacity-80` : ' text-gray-400 cursor-not-allowed'}`}>Continue</button>
                            </div>
                        )}
                        {currentStep === 2 && stakingType === 'Token Staking' && (
                            <div className="space-y-6">
                                <div><h2 className="text-2xl font-bold text-white mb-2">Select Staked Token</h2></div>
                                ...
                                <div className="flex gap-3">
                                    <button onClick={() => {setCurrentStep(1); setSelectedPool(null); setSelectToken('');}} className="flex-1 py-3 border border-gray-500 text-gray-300 rounded-lg font-medium transition-colors">Back</button>
                                    <button onClick={() => selectToken && setCurrentStep(3)} disabled={!selectToken} className={`flex-1 py-3 rounded-lg font-medium transition-all ${selectToken ? `${currentTheme.text} ${currentTheme.bg} border ${currentTheme.accent} hover:opacity-80` : 'border border-gray-500 text-gray-400 cursor-not-allowed'}`}>Continue</button>
                                </div>
                            </div>
                        )}
                        {currentStep === 2 && stakingType === 'Concentrate Liquidity Staking' && (
                            <div className="space-y-6">
                                ...
                                <div className="relative">
                                    ...
                                    <input ref={searchRef} type="text" placeholder="Search pools..." className={`w-full pl-10 pr-4 py-3  border ${currentTheme.border} ${currentTheme.hover} rounded-lg text-white placeholder-gray-400 focus:outline-none focus:${currentTheme.border}`} />
                                </div>
                                <div className="space-y-3">
                                    {validPools.map((pool) => (
                                        <div key={pool.id} onClick={() => setSelectedPool(pool)} className={`p-4 border rounded-lg cursor-pointer transition-all ${selectedPool?.id === pool.id ? `${currentTheme.accent} ${currentTheme.bg}` : `${currentTheme.border} ${currentTheme.hover}`}`}>
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div><h3 className="font-medium text-white">{pool.name}</h3></div>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                                ...
                                <div className="w-full space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {fees.map((fee) => (
                                            <button key={fee} onClick={() => toggleFee(fee)} className={`px-4 py-2 rounded-full border text-sm transition ${poolFees.includes(fee) ? `${currentTheme.bg} ${currentTheme.text} ring-2 ring-offset-1 ring-primary` : ' text-gray-400 hover:bg-gray-700 hover:text-gray-300'}`}>{Number(fee) / 10000}%</button>
                                        ))}
                                    </div>
                                    <p className="text-sm text-gray-400">Selected {poolFees.length} fee tier{poolFees.length !== 1 && 's'}.</p>
                                </div>
                                <div className="flex gap-3">
                                    <button onClick={() => {setCurrentStep(1); setSelectedPool(null); setSelectToken('');}} className="flex-1 py-3 border border-gray-500 text-gray-300 rounded-lg font-medium hover:bg-gray-600 transition-colors">Back</button>
                                    <button onClick={() => selectedPool && setCurrentStep(3)} disabled={!selectedPool} className={`flex-1 py-3 rounded-lg font-medium transition-all ${selectedPool ? `${currentTheme.text} ${currentTheme.bg} border ${currentTheme.accent} hover:opacity-80` : 'border border-gray-500 text-gray-400 cursor-not-allowed'}`}>Continue</button>
                                </div>
                            </div>
                        )}
                        {currentStep === 3 && (
                            <div className="space-y-6 pb-[60px]">
                                ...
                                {stakingType === 'Token Staking' ? (
                                    <>
                                        <div className="space-y-4">
                                            ...
                                            <Popover open={openReward} onOpenChange={setOpenReward}>
                                                ...
                                                <PopoverContent className="w-full p-0 z-50">
                                                    <Command>
                                                        <CommandInput ref={commandInputRef} placeholder="Search tokens..." value={searchInput} onValueChange={setSearchInput} />
                                                        <CommandList>
                                                            <CommandEmpty>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="space-y-3">
                                                <div className={`p-4 rounded-lg border cursor-pointer ${selectLockOption === '0' ? currentTheme.bg : ''}`} onClick={() => setSelectLockOption('0')}>
                                                ...
                                                <div className={`p-4 rounded-lg border cursor-pointer ${selectLockOption === '1' ? currentTheme.bg : ''}`} onClick={() => setSelectLockOption('1')}>
                                                ...
                                                <div className={`p-4 rounded-lg border cursor-pointer ${selectLockOption === '2' ? currentTheme.bg : ''}`} onClick={() => setSelectLockOption('2')}>
                                                    <div className="font-medium text-white">Multiple Lock Time</div>
                                                    <p className="text-sm text-gray-400">Users can choose from multiple lock durations predefined by the creator. The creator can assign different reward multipliers based on the selected duration.</p>
                                                </div>
                                            </div>
                                            {selectLockOption === '1' && (
                                                    ...
                                                    {multiLocks.map((item, index) => (
                                                        <div key={index} className="flex flex-col md:flex-row gap-4 items-start md:items-center /30 p-4 rounded-lg border">
                                                            ...
                                                            <div className="w-full md:w-1/3">
                                                                ...
                                                                <div className='w-full p-2 rounded-md border  text-white'>{convertBlocksToTimeString(Number(multiLocks[index].time), avgBlockTime)}</div>
                                                            </div>
                                                            <div className="w-full md:w-1/3">
                                                            ...
                                                            <button type="button" className="mt-2 md:mt-6 text-sm text-red-500 hover:text-red-700" onClick={() => setMultiLocks(multiLocks.filter((_, i) => i !== index))}>🗑 Remove</button>
                                                        </div>
                                                    ))}
                                                    <button type="button" className="px-4 py-2 rounded-md border text-sm font-medium hover:bg-gray-700 hover:text-white" onClick={() => setMultiLocks([...multiLocks, { time: '17280', multiplier: '120' }])}>+ Add</button>
                                                </div>
                                            )}
                                        </div>
                                        <div className="space-y-4">
                                            <label className="block text-sm font-medium text-gray-300 mb-2">First Block Reward</label>
                                            <DateTimePicker onSelect={(timestamp) => {
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <select value={duration} onChange={(e) => setDuration(e.target.value)} className="w-full px-4 py-3 bg-gray-700 border rounded-lg text-white focus:outline-none focus:border-gray-500">
                                                {[7, 14, 30, 90, 365].map((day) => {
                                                    const blocks = Math.ceil((day * 86400) / avgBlockTime);
                                                    return (<option key={day} value={blocks}>{`${blocks.toLocaleString()} Blocks ( ~${day} Days )`}</option>)
                                                })}
                                    ...
                                    <>
                                        <div className="grid gap-6">
                                            ...
                                            <div className="space-y-4">
                                                <label className="block text-sm font-medium text-gray-300 mb-2">First Block Reward</label>
                                                <DateTimePicker onSelect={(timestamp) => {const date = new Date(timestamp); setStartDate({timestamp, formatted: date.toLocaleString('en-US')});}} />
                                            </div>
                                            <div className="space-y-4">
                                                <label className="block text-sm font-medium text-gray-300 mb-2">Reward Duration (Blocks)</label>
                                                <select value={duration} onChange={(e) => setDuration(e.target.value)} className="w-full px-4 py-3 bg-gray-700 border rounded-lg text-white focus:outline-none focus:border-gray-500">
                                ...
                                <div className="flex gap-3">
                                    <button onClick={() => setCurrentStep(2)} className="flex-1 py-3 border border-gray-500 text-gray-300 rounded-lg font-medium  transition-colors">Back</button>
                                    <button onClick={() => setCurrentStep(4)} disabled={!rewardToken || !rewardAmount || !startDate} className={`flex-1 py-3 rounded-lg font-medium transition-all ${rewardToken && rewardAmount && startDate ? `${currentTheme.text} ${currentTheme.bg} border ${currentTheme.accent} hover:opacity-80` : 'border border-gray-500 text-gray-400 cursor-not-allowed'}`}>Continue</button>
                                </div>
                            </div>
                        )}
                        {currentStep === 4 && (
                            <div className="space-y-6">
                                <div>
                                    <h2 className="text-3xl font-bold text-white mb-2 tracking-wide" style={{ letterSpacing: '0.04em' }}>Review Farm Details</h2>
                                    <p className="text-gray-400 text-base tracking-normal" style={{ letterSpacing: '0.015em' }}>Confirm your farming program configuration</p>
                                </div>
                                <div className="space-y-4">
                                    <div className="p-4 border border-gray-600 rounded-lg">
                                        <h3 className="font-semibold text-white mb-3 text-xl tracking-wide" style={{ letterSpacing: '0.03em' }}>Program Information</h3>
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            <div>
                                                <div className="text-gray-400 ml-2 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>
                                                    {selectLockOption === '1' ?
                                                        "Lock : " :
                                                        selectLockOption === '2' ? "Multiple Lock : " : "Staking : "
                                                    }
                                                    <span className={`${currentTheme.text} tracking-wide`} style={{ letterSpacing: '0.025em', fontSize: '0.9rem' }}>{selectTokenInfo?.name || selectedPool?.name}</span>{" "}
                                                    → Earn{" "}
                                                    <span className={`${currentTheme.text} tracking-wide`} style={{ letterSpacing: '0.025em', fontSize: '0.9rem' }}>{rewardTokenInfo?.name}</span>
                                                </div>
                                                <div className="text-gray-400 ml-2 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>
                                                    Staked Duration :{" "}
                                                    <span className={`${currentTheme.text} tracking-wide`} style={{ letterSpacing: '0.025em', fontSize: '0.9rem' }}>{parseInt(effectiveDuration).toLocaleString()}</span>{" "}
                                                    Blocks (~ {convertBlocksToTimeString(Number(effectiveDuration), avgBlockTime)})
                                                </div>
                                                <div className="text-gray-400 ml-2 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>
                                                    Chain :{" "}
                                                    <span className={`${currentTheme.text} tracking-wide`} style={{ letterSpacing: '0.025em', fontSize: '0.9rem' }}>{currentTheme.name}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="p-4 border border-gray-600 rounded-lg">
                                        <h3 className="font-semibold text-white mb-3 text-xl tracking-wide" style={{ letterSpacing: '0.03em' }}>Pool Information</h3>
                                        <div>
                                            <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>{stakingType === "Token Staking" ? "Token Staked:" : "Pool:"}</span>
                                            <span className="text-white ml-2 tracking-wide" style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{stakingType === "Token Staking" ? selectTokenInfo?.name : selectedPool?.name}</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-sm text-gray-400 tracking-normal" style={{ letterSpacing: '0.015em' }}>
                                            <span>Contract Address:</span>
                                            <button onClick={() => copyToClipboard((stakingType === "Token Staking" ? selectToken : selectedPool?.poolAddress) || '', "stakingCa")} className="flex items-center gap-1 text-white transition-colors">
                                                <span>{stakingType === "Token Staking" ? selectToken : selectedPool?.poolAddress || "N/A"}</span>
                                                {copiedAddress === "stakingCa" ? <CopyCheck size={16} /> : <Copy size={16} />}
                                            </button>
                                        </div>
                                        {stakingType !== "Token Staking" && (
                                            <div>
                                                <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>Fee:</span>
                                                <span className={`${currentTheme.text} ml-2 tracking-wide`} style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>
                                                    {poolFees.map((fee) => Number(fee)).sort((a, b) => a - b).map((fee) => `${(fee / 10000).toFixed(2)}%`).join(", ")}
                                                </span>
                                            </div>
                                        )}
                                        <div className="space-y-4">
                                            <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>Lock Durations :{" "}</span>
                                            {selectLockOption !== "1" && selectLockOption !== "2" && (<span className="text-white tracking-wide" style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>No Lock</span>)}
                                            {selectLockOption === "1" && (<span className="text-white tracking-wide" style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{singleUnlockTime} Blocks</span>)}
                                            {selectLockOption === "2" && (
                                                    ...
                                                    {multiLocks.map((item, index) => (
                                                        <div key={index} className="space-y-1 ml-2 my-2 rounded-lg ">
                                                            <div className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>
                                                                Unlock Time (Blocks) :{" "}
                                                                <span className="text-white tracking-wide" style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{item.time || "-"}</span>{" "}
                                                                ({convertBlocksToTimeString(Number(multiLocks[index].time), avgBlockTime)})
                                                            </div>
                                                            <div className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>
                                                                Power Multiplier (%) :{" "}
                                                                <span className="text-white tracking-wide" style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{item.multiplier || "-"}</span>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                        ...
                                        <div>
                                            <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>Block reward range:</span>
                                            <span className="ml-2 text-white tracking-wide" style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{estimatedStartBlock}{" - "}{Number(estimatedStartBlock) + Number(effectiveDuration)}</span>
                                        </div>
                                        <div>
                                            <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>Maximum Lock per User :{" "}</span>
                                            <span className={`ml-2 text-white tracking-wide`} style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{Number(maxUserToken) === 0 ? "Unlimited" : Number(maxUserToken)} {rewardTokenInfo?.name}{" "}</span>
                                        </div>
                                        <div>
                                            <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>Maximum Pool Lock Token :{" "}</span>
                                            <span className={`ml-2 text-white tracking-wide`} style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{Number(maxPoolToken) === 0 ? "Unlimited" : Number(maxPoolToken)} {rewardTokenInfo?.name}{" "}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="p-4 border border-gray-600 rounded-lg">
                                    <h3 className="font-semibold text-white mb-3 text-xl tracking-wide" style={{ letterSpacing: '0.03em' }}>Reward Information</h3>
                                    <div>
                                        <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>Reward Token :{" "}</span>
                                        <span className="text-white ml-2 tracking-wide" style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{rewardTokenInfo?.name} ({rewardTokenInfo?.symbol})</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-sm text-gray-400 tracking-normal" style={{ letterSpacing: '0.015em' }}>
                                        <span>Contract Address :</span>
                                        <button onClick={() => copyToClipboard(rewardToken, "reward")} className="flex items-center gap-1 text-white transition-colors">
                                            <span>{rewardToken}</span>
                                            {copiedAddress === "reward" ? <CopyCheck size={16} /> : <Copy size={16} />}
                                        </button>
                                    </div>
                                    <div>
                                        <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>Total Reward {" :"}</span>
                                        <span className={`ml-2 text-white tracking-wide`} style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{Number(rewardAmount).toLocaleString()} {rewardTokenInfo?.symbol}</span>
                                    </div>
                                    <div>
                                        <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>
                                            Reward Durations {" : "}
                                            <span className='text-white'> {Number(effectiveDuration).toLocaleString()} Blocks ({convertBlocksToTimeString(Number(effectiveDuration), avgBlockTime)})</span>
                                        </span>
                                        <span className={`ml-2 text-white tracking-wide`} style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{" "}</span>
                                    </div>
                                    <div>
                                        <span className="text-gray-400 text-sm tracking-normal" style={{ letterSpacing: '0.015em' }}>Emission rate :{" "}</span>
                                        <span className="text-white tracking-wide" style={{ letterSpacing: '0.02em', fontSize: '0.9rem' }}>{getRewardRatePerDay(rewardAmount, effectiveDuration, avgBlockTime)} {rewardTokenSymbol || "tokens"}/day</span>
                                    </div>
                                </div>
                                <div className="flex gap-3">
                                    <button onClick={() => setCurrentStep(3)} className="flex-1 py-3 border border-gray-500 text-gray-300 rounded-lg font-medium tracking-wide transition-colors">Back</button>
                                    <button className={`flex-1 py-3 rounded-lg font-medium tracking-wide transition-all ${currentTheme.text} ${currentTheme.bg} border ${currentTheme.accent} hover:opacity-80`} onClick={()=> createPool()}>Create Farm</button>
                                </div>
                            </div>
                        )}
                    </div>
