...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!Number.isFinite(latestPrice) || !latestPrice || latestPrice <= 0) return NextResponse.json({ buckets: [], latest: { price: null }, window: { span, min: windowMin, max: windowMax }, chainId, marketId });
        ...
        const buckets = (maxVal > 0) ? sums.map((v, i) => ({ ratio: i / Math.max(1, bucketCount - 1), height: Math.min(100, Math.max(10, (v / maxVal) * 100)), value: v })) : sums.map((_, i) => ({ ratio: i / Math.max(1, bucketCount - 1), height: 0, value: 0 }))
        return NextResponse.json({buckets, latest: { price: latestPrice }, window: { span, min: windowMin, max: windowMax }, bucketCount, chainId, marketId})
    } catch (err: any) {
