...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (tokenAgg.error)
        return NextResponse.json({ error: tokenAgg.error.message }, { status: 500 });
        const traderAgg = await supabase.rpc('top_volume_traders', { chain_i: chainId, limit_i: limit });
        if (traderAgg.error)
        return NextResponse.json({ error: traderAgg.error.message }, { status: 500 });
        const profitAgg = await supabase.rpc('top_profit_traders', { chain_i: chainId, limit_i: limit });
        if (profitAgg.error)
        return NextResponse.json({ error: profitAgg.error.message }, { status: 500 });
        const profitTraders = (profitAgg.data || []).map((r: any) => ({sender: (r as any).sender, value: Number((r as any).value || 0), latest_ts: (r as any).latest_ts ?? null}));
        const degenAgg = await supabase.rpc('top_degen_traders', { chain_i: chainId, limit_i: limit });
        if (degenAgg.error)
        return NextResponse.json({ error: degenAgg.error.message }, { status: 500 });
        ...
        return NextResponse.json({
            volumeTokens: (tokenAgg.data || []).map((r: any) => ({token_address: (r as any).token_address, value: Number((r as any).value || 0), latest_ts: (r as any).latest_ts ?? null})),
            volumeTraders: (traderAgg.data || []).map((r: any) => ({sender: (r as any).sender, value: Number((r as any).value || 0), latest_ts: (r as any).latest_ts ?? null})),
            profitTraders,
            degenTraders: (degenAgg.data || []).map((r: any) => ({sender: (r as any).sender, value: Number((r as any).value || 0), latest_ts: (r as any).latest_ts ?? null})),
            tokens: tokenMeta,
            chainId,
            limit,
        });
