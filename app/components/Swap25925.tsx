...
export default function Swap25925({
    ...
    const [bestRateRoutes, setBestRateRoutes] = React.useState<{
    amountOut: bigint;
    path: string[];
    providerId: bigint;
    feeTiers: unknown[];
    ...
    const [totalAmountOut, setTotalAmountOut] = React.useState<string>(""); // final output summary
    ...
    const wrap = async () => {
        ...
        try {
            if (tokenA.value.toUpperCase() === tokens[0].value.toUpperCase()) {
                ...
                let allowanceA = await readContract(config, { ...erc20ABI, address: tokenA.value as '0xstring', functionName: 'allowance', args: [address as '0xstring', tokenA.value] })
                if (allowanceA < parseEther(amountA)) {
                    const { request } = await simulateContract(config, { ...erc20ABI, address: tokenA.value as '0xstring', functionName: 'approve', args: [tokenA.value, parseEther(amountA)] })
    ...
    const CMswap = async () => {
        ...
        try {
            ...
            if (tokenB.value.toUpperCase() === tokens[0].value.toUpperCase()) {
                let allowance = await readContract(config, { ...erc20ABI, address: tokens[1].value as '0xstring', functionName: 'allowance', args: [address as '0xstring', tokens[1].value] })
                if (allowance < r) {
                    const { request } = await simulateContract(config, { ...erc20ABI, address: tokens[1].value as '0xstring', functionName: 'approve', args: [tokens[1].value, r as bigint] })
                    const h = await writeContract(config, request)
                    await waitForTransactionReceipt(config, { hash: h })
                }
                let { request } = await simulateContract(config, {
...
{mode === "Auto" && bestRateRoutes.length > 0 && (
  <div className="mt-6 p-4 border border-[#00ff9d]/20 rounded bg-[#0a0b1e]/60 text-xs font-mono text-gray-300">
    <div className="mb-2 text-[#00ff9d] font-semibold">
      1 {tokenA.name} =&gt; {totalAmountOut} {tokenB.name}
    </div>
    <div className="space-y-2">
      {bestRateRoutes.map((route, index) => (
        <div key={index} className="pl-4 border-l border-[#00ff9d]/40">
          <div className="flex items-center justify-between">
            <div>
              ↳ {formatEther(route.amountIn).slice(0, 6)} KKUB ➝{" "}
              <span className="text-[#00ff9d]">
                {formatEther(route.amountOut).slice(0, 6)} KUSDT
              </span>
            </div>
            <span className="text-gray-500">Provider {Number(route.providerId)}</span>
          </div>
