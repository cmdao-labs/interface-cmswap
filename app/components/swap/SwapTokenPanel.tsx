...
/**
 * Shared token selection panel used by the swap variants.
 * Handles rendering the token address input, amount input, and popover picker.
 */
export function SwapTokenPanel<TToken extends SwapTokenOption>({
    label,
    tokenAddress,
                ...
                <Popover open={popoverOpen} onOpenChange={onPopoverOpenChange}>
                    <PopoverTrigger asChild>
                        <Button
                            ...
                            className="w-[180px] bg-[#162638] hover:bg-[#1e3048] text-white border-[#00ff9d]/20 flex items-center justify-between h-10 cursor-pointer"
                        >
                            <div className="gap-2 flex flex-row items-center justify-center">
                                <div className="w-5 h-5 rounded-full bg-[#00ff9d]/20">
                                    <span className="text-[#00ff9d] text-xs">
                                        {selectedToken.logo && selectedToken.logo !== '../favicon.ico'
                                            ? <img alt="" src={selectedToken.logo} className="size-5 shrink-0 rounded-full" />
                                            : '?'}
                                    </span>
                                </div>
