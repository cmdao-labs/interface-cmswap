...
export function useSwap8899PoolData({
    config,
    address,
    tokens,
    tokenA,
    ...
    React.useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const tvl_10000 = currPrice_10000 !== 0 && currPrice_10000 !== Infinity
                    ? Number(formatEther(tokenAamount_10000)) * (1 / currPrice_10000) + Number(formatEther(tokenBamount_10000))
                    : 0
                ...
                const tvl_3000 = currPrice_3000 !== 0 && currPrice_3000 !== Infinity
                    ? Number(formatEther(tokenAamount_3000)) * (1 / currPrice_3000) + Number(formatEther(tokenBamount_3000))
                    : 0
                ...
                const tvl_500 = currPrice_500 !== 0 && currPrice_500 !== Infinity
                    ? Number(formatEther(tokenAamount_500)) * (1 / currPrice_500) + Number(formatEther(tokenBamount_500))
                    : 0
                ...
                const tvl_100 = currPrice_100 !== 0 && currPrice_100 !== Infinity
                    ? Number(formatEther(tokenAamount_100)) * (1 / currPrice_100) + Number(formatEther(tokenBamount_100))
                    : 0
                    ...
                    for (let i = 0; i < results.length; i += 2) {
                        ...
                        if (first !== '0x0000000000000000000000000000000000000000' && second !== '0x0000000000000000000000000000000000000000') {
                            ...
                            const alt0Price = altToken0.toUpperCase() === tokenAvalue.toUpperCase()
                                ? (Number(altPoolState[1].result !== undefined ? altPoolState[1].result[0] : BigInt(0)) / (2 ** 96)) ** 2
                                : 1 / ((Number(altPoolState[1].result !== undefined ? altPoolState[1].result[0] : BigInt(0)) / (2 ** 96)) ** 2)
                            ...
                            const alt1Price = altToken1.toUpperCase() === tokenBvalue.toUpperCase()
                                ? (Number(altPoolState[3].result !== undefined ? altPoolState[3].result[0] : BigInt(0)) / (2 ** 96)) ** 2
                                : 1 / ((Number(altPoolState[3].result !== undefined ? altPoolState[3].result[0] : BigInt(0)) / (2 ** 96)) ** 2)
                ...
                if (feeSelect === 500 && tvl_500 < 1e-9) {
                    await ensureAltRoute(500)
                }
                if (feeSelect === 100 && tvl_100 < 1e-9) {
                    await ensureAltRoute(100)
                }

                // GameSwap calculations
                const fetchGameSwapTvl = async () => {
                    if (
                        ...
                        const tokenAamount_JCLP = tokenAvalue.toUpperCase() === tokens[1].value.toUpperCase()
                            ? (await getBalance(config, { address: JCLP })).value
                            : loadPoolState[19].result !== undefined ? loadPoolState[19].result : BigInt(0)
                        const tokenBamount_JCLP = tokenBvalue.toUpperCase() === tokens[3].value.toUpperCase()
                            ? loadPoolState[19].result !== undefined ? loadPoolState[19].result : BigInt(0)
                            : (await getBalance(config, { address: JCLP })).value
                        ...
                        const tokenAamount_JULP = tokenAvalue.toUpperCase() === tokens[1].value.toUpperCase()
                            ? (await getBalance(config, { address: JULP })).value
                            : loadPoolState[17].result !== undefined ? loadPoolState[17].result : BigInt(0)
                        const tokenBamount_JULP = tokenBvalue.toUpperCase() === tokens[2].value.toUpperCase()
                            ? loadPoolState[16].result !== undefined ? loadPoolState[16].result : BigInt(0)
                            : (await getBalance(config, { address: JULP })).value
                ...
                try {
                    ...
                    const [{ result: jibPair }] = await readContracts(config, {
                        contracts: [{ ...CMswapUniSmartRouteContract, functionName: 'getPairAddress', args: [swapTokenA, swapTokenB] }],
                    })
