...
export async function wrapNativeToken({
    ...
    wrappedTokenAddress,
    amount,
}: WrapNativeOptions): Promise<`0x${string}`> {
    const hash = await sendTransaction(config, { to: wrappedTokenAddress as `0x${string}`, value: amount })
    await waitForTransactionReceipt(config, { hash })
    return hash
}

export async function unwrapWrappedToken({
    config,
    contract,
    amount,
    functionName = 'withdraw',
    args,
}: UnwrapNativeOptions): Promise<`0x${string}`> {
...
export async function ensureTokenAllowance({
    ...
    spender,
    requiredAmount,
    allowanceFunctionName = 'allowance',
    approveFunctionName = 'approve',
    approveArgs,
}: EnsureTokenAllowanceOptions): Promise<void> {
    ...
    const allowance = await readContract(config, {
        ...
        functionName: allowanceFunctionName,
        args: [owner, spender] as never,
    } as any) as bigint
    ...
    const simulation = await simulateContract(config, {
        ...
        args: approvalArguments as never,
    } as any)
...
}: ExecuteRouterSwapOptions): Promise<ExecuteRouterSwapResult> {
    if (!path && fee === undefined) {
        throw new Error('executeRouterSwap requires a fee when no path is provided.')
    }

    if (amountIn <= BigInt(0)) {
        throw new Error('executeRouterSwap requires amountIn greater than zero.')
    }

    if (amountOutMinimum < BigInt(0)) {
        throw new Error('executeRouterSwap requires amountOutMinimum to be non-negative.')
    }

    if (!recipient) {
        throw new Error('executeRouterSwap requires a valid recipient address.')
    }
