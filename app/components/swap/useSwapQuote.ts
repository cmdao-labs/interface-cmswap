...
export function useSwapQuote<TToken extends SwapTokenWithValue>(options: UseSwapQuoteOptions<TToken>) {
    const {config, contract, tokens, nativeTokenIndex = 0, wrappedTokenIndex = 1} = options
    ...
    const resolveTokenAddress = React.useCallback(
        (token: TToken) => {
            ...
            if (!rawValue) throw new Error('Token value is undefined. Ensure tokens include a value property.');
            if (nativeTokenValue && wrappedTokenValue && rawValue.toLowerCase() === nativeTokenValue.toLowerCase()) return wrappedTokenValue;
    ...
    const quoteExactInputSingle = React.useCallback(
        async (params: QuoteExactInputSingleParams<TToken>): Promise<QuoteResult | null> => {
            const {tokenIn, tokenOut, fee, amount, parseAmount, sqrtPriceLimitX96 = BigInt(0), suppressErrors = false} = params
            ...
            try {
                ...
                const response = await simulateContract(config, {
                    ...
                    args: [{tokenIn: normalizedIn, tokenOut: normalizedOut, amountIn, fee, sqrtPriceLimitX96}] as const,
                } as const)
    ...
    const quoteExactInput = React.useCallback(
        async (params: QuoteExactInputParams<TToken>): Promise<QuoteResult | null> => {
            const {path, tokenIn, amount, parseAmount, suppressErrors = false} = params
            if (!amount || Number(amount) === 0) return null
            try {
                const amountIn = parseAmount(amount, tokenIn)
                if (amountIn <= BigInt(0)) return null
    ...
    return {resolveTokenAddress, quoteExactInputSingle, quoteExactInput}
}
