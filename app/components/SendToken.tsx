...
const [showScanner, setShowScanner] = useState(false);
  ...
  async function resolveNameIfNeeded(input: string) {
    ...
    try {
      ...
      return resolved ?? input as '0xstring'; // ถ้า resolve ไม่ได้ ก็ใช้ input ตรง
    } catch (e) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (token.value === "0xnative" as "0xstring") {
        ...
        const { request } = await simulateContract(config, {
          ...
          args: [toResolved as '0xstring', BigInt(Number(amount) * 1e18)],
        });
    ...
    // factor สำหรับ truncate ทศนิยมที่ต้องการ
    const factor = BigInt(10) ** BigInt(18 - decimals); // 18 คือ decimals ของ token
    const truncated = bal / factor;

    // แปลงเป็น string แบบ integer + decimal
  ...
  if (value === "0xnative") {
    ...
    setBalance(
      (Number(bal.value) / 1e18).toLocaleString(undefined, { maximumFractionDigits: 6 })
    );
  } else {
    ...
    setBalance(
      (Number(bal) / 1e18).toLocaleString(undefined, { maximumFractionDigits: 6 })
    );
  }
};
    React.useEffect(() => {
