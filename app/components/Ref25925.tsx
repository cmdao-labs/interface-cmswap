...
import { tokens, erc20ABI, kap20ABI, cmSwapRefProgram, cmSwapRefProgramContract } from "@/app/lib/25925";
        ...
        <div className="min-h-screen min-w-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className={`bg-black/40 backdrop-blur-lg border ${theme.border} rounded-2xl p-6 ${theme.glow} shadow-2xl group hover:scale-105 transition-transform`}>
                        <div className="flex items-center justify-between mb-4">
                            <div className={`p-3 bg-gradient-to-r ${theme.primary} rounded-xl ${theme.glow} shadow-lg`}>
                                        ...
                                        .map((referral, index) => (
                                            <div key={index} className={`flex items-center justify-between p-4 bg-black/30 rounded-xl border ${theme.border} hover:bg-black/50 transition-colors`}>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="font-mono text-gray-300 break-all cursor-pointer hover:underline" onClick={() => navigator.clipboard.writeText(referral.referral)} title="Click to copy full address">
                                                        {referral.referral.slice(0, 6) + "..." + referral.referral.slice(-4)}
                                                    </span>
                                                </div>
                                                <div className={`px-3 py-1 bg-gradient-to-r ${theme.secondary} rounded-full text-[10px] lg:text-[14px] font-bold`}>
                                    ...
                                    <div className="flex flex-wrap justify-center items-center gap-2 mt-6 text-sm">
                                        ...
                                        {Array.from({ length: totalPages }).map((_, idx) => {
                                            ...
                                            if (pageNum === 1 || pageNum === totalPages || Math.abs(pageNum - currentPage) <= 1) {
                                                return (<button key={pageNum} onClick={() => setCurrentPage(pageNum)} className={`px-3 py-1 rounded font-bold ${pageNum === currentPage ? "bg-gradient-to-r " + theme.primary + " text-black shadow-md" : "bg-black/30 text-white hover:bg-black/50"}`}>{pageNum}</button>);
                                            }
            ...
            {isLoading && 
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center">
                    <div className={`animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-${theme.accent}`} />
                </div>
            }
        </div>
    );
